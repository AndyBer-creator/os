...
_drv_swcore_jtag_intf_8380_set(uint32 unit, drv_swcore_jtag_intf_t intf_sel)
{
    ...
    data &= ~(RTL8380_GMII_INTF_SEL_JTAG_SEL_MASK);
    data |= (intf_sel << RTL8380_GMII_INTF_SEL_JTAG_SEL_OFFSET);
...
static int32
_drv_swcore_jtag_intf_9310_set(uint32 unit, drv_swcore_jtag_intf_t intf_sel)
{
	uint32 data;

    ioal_mem32_read(unit, RTL9310_MAC_L2_GLOBAL_CTRL2_ADDR, &data);
    
    data &= ~(RTL9310_MAC_L2_GLOBAL_CTRL2_JTAG_SEL_MASK);
    
    if(intf_sel == JTAG_INTF_JTAG)
    	data |= RTL9310_MAC_L2_GLOBAL_CTRL2_JTAG_SEL_MASK;
	
    ioal_mem32_write(unit, RTL9310_MAC_L2_GLOBAL_CTRL2_ADDR, data);
    return RT_ERR_OK;
}
#endif
...
drv_swcore_jtag_intf_set(uint32 unit, drv_swcore_jtag_intf_t intf_sel)
{
    ...
    case RTL9310_FAMILY_ID:
        _drv_swcore_jtag_intf_9310_set(unit, intf_sel);
        break;
   #endif
    default:
...
_drv_swcore_jtag_intf_8380_get(uint32 unit, drv_swcore_jtag_intf_t * pIntf_sel)
{
    ...
    *pIntf_sel = ((reg_value) & RTL8380_GMII_INTF_SEL_JTAG_SEL_MASK);
    return RT_ERR_OK;
}
#endif
