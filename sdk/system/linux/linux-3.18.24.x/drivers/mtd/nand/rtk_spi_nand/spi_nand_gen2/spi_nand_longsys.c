...
#define DID_FS36ND04G_S2Y2QWFI000 (0xEC11)
#define DID_F35SQA002G            (0x7272)
...
spi_nand_cmd_info_t longsys_x4_cmd_info = {
...
longsys_ecc_encode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs) { return; }


__SECTION_RUNTIME s32_t
longsys_ecc_decode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs)
{
    ...
    if (0==ecst) { //no error
        ...
        res = ECC_CTRL_ERR;
    } else if (1==ecst) { // error and corrected
        res = 4;          // return max ecc bits / 512 bytes
    } else {              // reserved
        res = 0;
    }
...
spi_nand_flash_info_t longsys_chip_info[] = {
    {
        ...
        .man_id              = MID_LONGSYS,
        .dev_id              = DID_FS36ND04G_S2Y2QWFI000,
        ...
        ._model_info         = VZERO,
    #else
        ._ecc_encode         = longsys_ecc_encode,
        ._ecc_decode         = longsys_ecc_decode,
        ._reset              = nsu_reset_spi_nand_chip,
...
probe_longsys_spi_nand_chip(void)
{
    ...
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(longsys_chip_info) ; i++){
        if(longsys_chip_info[i].dev_id == did)
        {
        #if __DEVICE_REASSIGN
          #if __DEVICE_USING_QIO
            longsys_chip_info[i]._cmd_info = &longsys_x4_cmd_info;
          #elif __DEVICE_USING_DIO
            ...
            if(ECC_USE_ODE==longsys_chip_info[i]._ecc_ability) {
                nsu_model_info._page_read_ecc  = _nsu_page_read_with_ode_ptr;
                nsu_model_info._page_write_ecc = _nsu_page_write_with_ode_ptr;
                longsys_chip_info[i]._ecc_encode= longsys_ecc_encode;
                longsys_chip_info[i]._ecc_decode= longsys_ecc_decode;
...
int
longsys_init_rest(spi_nand_flash_info_t * _spi_nand_info)
{
    u32_t cs=1;
