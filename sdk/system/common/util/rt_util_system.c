...
static const board_probe_gpio_t board_probe_pin_example = {
    ...
    .pin = {
            [0] = {HWP_GPIO_PROBE_USED_DEVICE_ID, 0x0, 30, GPIO_DIR_IN},  /*LSB*/
            [1] = {HWP_GPIO_PROBE_USED_DEVICE_ID, 0x0, 31, GPIO_DIR_IN},  /*MSB*/
            [2] = {HWP_GPIO_PROBE_USED_DEVICE_ID, 0x0, 32, GPIO_DIR_IN},  /*LSB*/
            [3] = {HWP_GPIO_PROBE_USED_DEVICE_ID, 0x0, 33, GPIO_DIR_IN},  /*MSB*/
    },
};
/*自定义的board id和hw profile board id 对应的映射关系*/
static const board_gpioValue_to_boardId_t board_probe_gpio_to_hwprofileId[] = {
	{
		.gpioBoardidValue = 0x00,
		.hw_profileBoardid = HWP_RTL9301_3x8218D_4XGE,
	},
	{
		.gpioBoardidValue = 0xFF,
		.hw_profileBoardid = 0x00,
	},
};
#endif
...
int32 rt_util_boardID_GPIO_get(uint32 *pPinValue)
{
    ...
    for(loop = 0; loop < board_probe_pin_example.total_gpio; loop++)
    {
        ...
        else if(board_probe_pin_example.pin[loop].gpio_device_id < GEN_GPIO_DEV_ID_END)
        {
			...
			//RT_INIT_MSG("\n%s %d GET device %d by GPIO pin: %d, value:%d \n", __func__, __LINE__, board_probe_pin_example.pin[loop].gpio_device_id, board_probe_pin_example.pin[loop].gpio_id, pin_value);
	...
	/*wt add 20210425 更加GPIO value 设置对应的hw profile board id*/
    for(i = 0; i < 8; i++)
    {
		if((board_probe_gpio_to_hwprofileId[i].gpioBoardidValue == 0xFF) && (board_probe_gpio_to_hwprofileId[i].hw_profileBoardid == 0x00))
			break;
		if(board_probe_gpio_to_hwprofileId[i].gpioBoardidValue == probe_value)
		{
			*pPinValue = board_probe_gpio_to_hwprofileId[i].hw_profileBoardid;
			break;
		}
    }
    //*pPinValue = probe_value;

    RT_LOG(LOG_INFO, (MOD_INIT), "GET board id by GPIO: PINs value = 0x%08x", pPinValue);
    RT_INIT_MSG("\n GET board id by GPIO: PINs value = 0x%08x \n", *pPinValue);
