...
#define ROM_SIZE (PARTITION_SIZE(FLASH_INDEX_LOADER) + PARTITION_SIZE(FLASH_INDEX_LOADER_BDINFO) + PARTITION_SIZE(FLASH_INDEX_SYSINFO) + PARTITION_SIZE(FLASH_INDEX_JFFS2_CFG) + PARTITION_SIZE(FLASH_INDEX_JFFS2_LOG) + PARTITION_SIZE(FLASH_INDEX_KERNEL) + PARTITION_SIZE(FLASH_INDEX_KERNEL2) + PARTITION_SIZE(FLASH_INDEX_OEMINFO))
#else
#define ROM_SIZE (PARTITION_SIZE(FLASH_INDEX_LOADER) + PARTITION_SIZE(FLASH_INDEX_LOADER_BDINFO) + PARTITION_SIZE(FLASH_INDEX_SYSINFO) + PARTITION_SIZE(FLASH_INDEX_JFFS2_CFG) + PARTITION_SIZE(FLASH_INDEX_JFFS2_LOG) + PARTITION_SIZE(FLASH_INDEX_KERNEL) + PARTITION_SIZE(FLASH_INDEX_OEMINFO))
#endif
...
#endif
struct rtk_partition_info_t rtk_flash_partition_size[5] = {
     [0] = {
     ...
     [1] = {
         ...
         .oeminfo_offset = (0x00f00000U),
         .oeminfo_size = (UNIT_SIZE*16)
#else
         .loader_offset = (0x00000000U),
         .loader_size = (UNIT_SIZE*14),
         .bdinfo_offset = (0x000e0000U),
     ...
     [2] = {
         ...
         .kernel2_size = (0x00d80000U),
         .oeminfo_offset = (0x01f00000U),
         .oeminfo_size = (UNIT_SIZE*16)
#else
         .loader_offset = (0x00000000U),
         .loader_size = (UNIT_SIZE*14),
    ...
    switch(partition_idx){
        ...
        case FLASH_INDEX_KERNEL2:			/* KERNEL2 */
            ...
            *size = rtk_flash_partition_size[size_idx].kernel2_size;
            break;
#endif
        case FLASH_INDEX_OEMINFO:           /* OEMINFO */
            *offset = rtk_flash_partition_size[size_idx].oeminfo_offset;
            *size = rtk_flash_partition_size[size_idx].oeminfo_size;
            break;
        default:
            break;
    }
...
{
    char filename[256];
    unsigned int port, portEnable;

    osal_memset(filename, 0, sizeof(filename));

#ifdef CONFIG_WARM_RESTART_CLOSE_PORT_8382
	/*mac port ctrl*/
	for(port = 0; port < 28; port++)
	{
		portEnable = REG32(0xBB00D560 + (port * 0x80));
		portEnable = (portEnable & (~0x3)) | 0x3;
		REG32(0xBB00D560 + (port * 0x80)) = portEnable; // close port 0
	}
#endif
