    ...
    *pSts = 0;
    ...
    switch (mode)
    {
        ...
        case RTK_MII_SGMII:
            ...
            break;
        case RTK_MII_2500Base_X:
            if (0x111 == linkInfo.sts && 0x111 == linkInfo.sts1)
                *pSts = 1;
            break;
        default:
            if (0x1 == linkInfo.sts)
...
int32
_phy_rtl9310_pairMedia_get(uint32 unit, uint32 sds, rtk_port_10gMedia_t *pPairMedia)
{
    uint32  pairSds, pairPort;
    int32   ret;
    ...
    pairPort = HWP_SDS_ID2MACID(unit, pairSds);

    RT_ERR_CHK(_phy_rtl9310_10gMedia_get(unit, pairPort, pPairMedia), ret);

    return ret;
}   /* end of _phy_rtl9310_pairMedia_get */
...
_phy_rtl9310_10gMedia_set(uint32 unit, rtk_port_t port, rtk_port_10gMedia_t media)
{
    ...
    RT_ERR_CHK(SDS_FIELD_W(unit, dSds, 0x1, 0x14, 8, 8, 0), ret);
    RT_ERR_CHK(SDS_FIELD_W(unit, dSds, 0x0, 0xE, 12, 12, 0), ret);
    ...
    switch (media)
    {
        ...
        case PORT_10GMEDIA_FIBER_1G:
            ...
            RT_ERR_HDL(SDS_FIELD_W(unit, asds, 0x6, 13, 6, 6, 1), ERR3, ret);
            RT_ERR_CHK(SDS_FIELD_W(unit, even_asds, 0x20, 0x12, 15, 0, 0x150F), ret);
            break;
        case PORT_10GMEDIA_FIBER_100M:
        ...
        case PORT_10GMEDIA_FIBER_2_5G:
            ...
            RT_ERR_CHK(SDS_FIELD_W(unit, even_asds, 0x20, 0x12, 15, 0, 0x350F), ret);
            break;
        default:
            ret = RT_ERR_INPUT;
            goto ERR3;
...
{
    rtk_port_10gMedia_t pairMedia = PORT_10GMEDIA_NONE;
    uint32  sds;
    int32   ret = RT_ERR_OK;
    ...
    /* check media if valid */
    if (PORT_10GMEDIA_FIBER_1G == media || PORT_10GMEDIA_FIBER_2_5G == media)
    {
        RT_ERR_CHK(_phy_rtl9310_pairMedia_get(unit, sds, &pairMedia), ret);

        if ((PORT_10GMEDIA_FIBER_1G == media && PORT_10GMEDIA_FIBER_2_5G == pairMedia) ||
                (PORT_10GMEDIA_FIBER_2_5G == media && PORT_10GMEDIA_FIBER_1G == pairMedia))
        {
            PHY_RTL9310_SEM_UNLOCK(unit);
...
phy_9310_linkChange_process(uint32 unit, rtk_port_t port, rtk_port_linkStatus_t linkSts)
{
    ...
    if(HWP_SDS_MODE(unit, sds) == RTK_MII_USXGMII_10GQXGMII)
    {
        if(sds <= 7)
        {
            switch(port % 8)
        ...
        RT_ERR_HDL(SDS_FIELD_R(unit, sds, 7, 17, bit, bit, &val), ERR, ret);

        if (linkSts == PORT_LINKUP && val == 0) //link up port with nWay disabled
        {
            //toggle the nWay status
            val = 1;
            RT_ERR_HDL(SDS_FIELD_W(unit, sds, 7, 17, bit, bit, val), ERR, ret);
            val = 0;
            RT_ERR_HDL(SDS_FIELD_W(unit, sds, 7, 17, bit, bit, val), ERR, ret);
        }
    }
