...
int32 dal_mango_construct_macConfig_init(uint32 unit)
{
    ...
    HWP_SDS_TRAVS(unit, sds)
    {
        ...
        //osal_printf("\n[macConf]: sds=%d, aSds=%d", sds, aSds);
        RT_ERR_CHK(drv_rtl9310_sds2XsgmSds_get(unit, sds, &xsg_sdsid_0), ret);
        //osal_printf("\n[macConf]: sds=%d, xsg_sdsid_0=%d", sds, xsg_sdsid_0);
...
static int32 _dal_mango_construct_init_usxgmii(uint32 unit, uint32 sds, uint32 aSds)
{
    ...
    else
    {
        ...
        RT_ERR_CHK(SDS_FIELD_W(unit, aSds, 0x7, 0x10, 15, 8, 0x60), ret);
        RT_ERR_CHK(SDS_FIELD_W(unit, aSds, 0x7, 0x10, 7, 0, op_code), ret);

        RT_ERR_CHK(hal_serdes_reg_set(unit, aSds, 0x6, 0x12, am_period), ret);
