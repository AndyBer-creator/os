...
{
    int32 ret;
    uint32 phy_data, phy_data2;
    rtk_port_t port_id;
    int32   phy_id;
    uint32 speed_val;
    uint8 speed;
    uint32 q_rst, frc_clk_5_3, frc_clk_13, frc_clk_15_12;
    ...
    if (HWP_8380_FAMILY_ID(unit))
    {
        ...
        /*If out of range means TX CRC Error*/
         ...
         {
            q_rst = TRUE;
            if ((HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8218D) || (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8218E))
            {
                /*Link Speed*/
                if ((ret = reg_array_field_read(unit, MAPLE_MAC_LINK_SPD_STSr, port, REG_ARRAY_INDEX_NONE, \
                    ...
                    return ret;
                speed = speed_val & 0x3;
                if ((ret = dal_phy_portReg_get(unit,  port,  0xc40,  21, &phy_data2)) != RT_ERR_OK)
                    return ret;
                /* speed and frc_clk check, if not match:
                 *  1) correct frc_clk value
                 *  2) clear counter
                 *  3) do not reset queue
                 */
                if (speed == 0x0) //10M
                {
                    frc_clk_5_3 = (phy_data2 & (0x7 << 3)) >> 3;
                    frc_clk_13 =  (phy_data2 & (0x1 << 13)) >> 13;
                    if (!((frc_clk_5_3 == 0x5) && (frc_clk_13 == 0x1)))
                    {
                        q_rst = FALSE;
                        phy_data2 = (phy_data2 & ~(0x7 << 3))  | (0x5 << 3);
                        phy_data2 = (phy_data2 & ~(0x1 << 13)) | (0x1 << 13);
                        if ((ret = dal_phy_portReg_set(unit,  port,  0xc40,  21, phy_data2)) != RT_ERR_OK)
                            return ret;
                        _dal_maple_port_phy_counter_clr_counter(unit, port);
                    }
                }
                else if (speed == 0x1) //100M
                {
                    frc_clk_5_3 = (phy_data2 & (0x7 << 3)) >> 3;
                    frc_clk_13 =  (phy_data2 & (0x1 << 13)) >> 13;
                    if (!((frc_clk_5_3 == 0x4) && (frc_clk_13 == 0x1)))
                    {
                        q_rst = FALSE;
                        phy_data2 = (phy_data2 & ~(0x7 << 3))  | (0x4 << 3);
                        phy_data2 = (phy_data2 & ~(0x1 << 13)) | (0x1 << 13);
                        if ((ret = dal_phy_portReg_set(unit,  port,  0xc40,  21, phy_data2)) != RT_ERR_OK)
                            return ret;
                        _dal_maple_port_phy_counter_clr_counter(unit, port);
                ...
                else if (speed == 0x2) //1000M
                {
                    frc_clk_15_12 = (phy_data2 & (0xF << 12)) >> 12;
                    if (frc_clk_15_12 != 0)
                    {
                        q_rst = FALSE;
                        phy_data2 = (phy_data2 & ~((0xF << 12)));
                        if ((ret = dal_phy_portReg_set(unit,  port,  0xc40,  21, phy_data2)) != RT_ERR_OK)
                            return ret;
                        _dal_maple_port_phy_counter_clr_counter(unit, port);
                    }
