...
rtk_switch_deviceInfo_get(uint32 unit, rtk_switch_devInfo_t *pDevInfo)
{
    ...
    /* 2.5GE */
    pDevInfo->qxge.portNum = HWP_2_5GE_PORT_COUNT(unit);
    osal_memset(&pDevInfo->qxge.portmask, 0, sizeof(rtk_portmask_t));
    for (i=0; i<RTK_MAX_NUM_OF_PORTS; i++) {
        if (HWP_PORT_EXIST(unit, i) && HWP_2_5GE_PORT(unit, i))
            RTK_PORTMASK_PORT_SET(pDevInfo->qxge.portmask, i);
    }/* end for */
    /* 10GE */
