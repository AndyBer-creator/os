...
static void 
vty_out_cpu_thread_history( struct cpu_thread_history *a)
{
#ifdef HAVE_RUSAGE
  printf("%7ld.%03ld %9d %8ld %9ld %8ld %9ld",
	  a->cpu.total/1000, a->cpu.total%1000, a->total_calls,
	  a->cpu.total/a->total_calls, a->cpu.max,
	  a->real.total/a->total_calls, a->real.max);
#else
  printf("%7ld.%03ld %9d %8ld %9ld",
...
static void
cpu_record_print(struct      thread_type filter)
{
...
#ifdef HAVE_RUSAGE
  osal_printf( "%21s %18s %18s%s",
  	  "", "CPU (user+system):", "Real (wall-clock):", VTY_NEWLINE);
#endif
  osal_printf( "Runtime(ms)   Invoked Avg uSec Max uSecs");
#ifdef HAVE_RUSAGE
  osal_printf(" Avg uSec Max uSecs");
#endif
  osal_printf("  Type  Thread%s", VTY_NEWLINE);
