...
#define MAX_MORE_LINES       CAPA_CLI_MAX_LINES_NUM
#define PRINT_MORE_NONE      0
#define PRINT_MORE_NORMAL    1
#define PRINT_MORE_PRESINGLE 2
#define PRINT_MORE_SINGLE    3
#define PRINT_MORE_FORBID    4

static struct termios    stored_settings;
static int        lines = 0;
static int        stopped = 0;
static int        max_line = 24;
...
vty_out_exit (void)
{
    stopped = 1;
    lines = 0;
    /* Make Ctrl+c will not print one more line */
    print_more = PRINT_MORE_NORMAL;
}

void vty_more_forbid(void)
{
    print_more = PRINT_MORE_FORBID;
}

void vty_more_normal(void)
{
    print_more = PRINT_MORE_NORMAL;
}

/*RTK: Detect keyboard input */
int vty_kbhit (void)
{
    struct timeval tv;
    fd_set rdfs;
...
vty_out (struct vty *vty, const char *format, ...)
{
    ...
    if (vty_shell (vty))
    {
        if (PRINT_MORE_FORBID == print_more)
        {
        ...
        else if (PRINT_MORE_NORMAL == print_more)
        {
            lines = 0;
            print_more = PRINT_MORE_NONE;
            ...
            /* Clear input buffer before read user input. */
            tcflush(STDIN_FILENO, TCIFLUSH);

            /* Keep detecting keyboard input */
            while(!vty_kbhit())
            {
                if (stopped)
                    break;
            }
            /* If not stop by ctrl+c, use getchar to retrieve input char from stdin buffer */
            if (!stopped)
                ch = getchar() & 0xFF;
            else
                ch = 'q';
            ...
            /* Detect 'Enter' key to print single line */
            else if (ch == 0xa)
            {
                va_start (args, format);

                if (NULL != strchr(format, '\n'))
                    print_more = PRINT_MORE_SINGLE;
                else
                    print_more = PRINT_MORE_PRESINGLE;
        ...
        else if (PRINT_MORE_SINGLE == print_more)
        {
            lines = 0;
        ...
        if (PRINT_MORE_NORMAL != print_more)
            vprintf (format, args);

        if (max_line > 0 && lines >= max_line && 1 == islineEnd)
            print_more = PRINT_MORE_NORMAL;
        else if (print_more == PRINT_MORE_PRESINGLE && 1 == islineEnd)
            print_more = PRINT_MORE_SINGLE;
...
int _input_str(char *buf, int size, char echo, int oversize)
{
    unsigned int pos = 0;
    char c = 0;
...
int banner_input_str(char *buf, int size, char echo, int oversize)
{
    unsigned int pos = 0;
    char c = 0;
...
int
input_str(char *buf, int size, char echo)
{
    return _input_str(buf, size, echo, FALSE);
}

int
input_str_oversize_break(char *buf, int size, char echo)
{
    return _input_str(buf, size, echo, TRUE);
}
         ...
         const char *format, struct timestamp_control *ctl, va_list va)
{
  ...
  if (write(vty->fd, buf, len) < 0)
    ...
    return -1;
      ...
      printf("%s: write failed to vty client fd %d, closing: %s",
...
void vty_maxline_set(int maxline)
{
    max_line = maxline;
...
vty_event (enum event event, int sock, struct vty *vty)
{
  ...
  switch (event)
    ...
    case VTY_READ:
      ...
      printf("Timeout treatment under VTY_READ\n");
      vty_out(vty, "Timeout treatment under VTY_READ\n");
    ...
    case VTY_TIMEOUT_RESET:
      if (vty->t_timeout)
    {
          printf("Timeout treatment under VTY_TIMEOUT_RESET1\n");

    vty_out(vty, "Timeout treatment under VTY_TIMEOUT_RESET1\n");
      thread_cancel (vty->t_timeout);
      vty->t_timeout = NULL;
    }
      if (vty->v_timeout)
    {
    printf("Timeout treatment under VTY_TIMEOUT_RESET2\n");
    vty_out(vty, "Timeout treatment under VTY_TIMEOUT_RESET2\n");
      vty->t_timeout =
        thread_add_timer (master, vty_timeout, vty, vty->v_timeout);
    }
      break;
