...
zerror (const char *fname, int type, size_t size)
{
  ...
  printf("%s : can't allocate memory for type-%d size %d: %s\n",
          fname, type, (int) size, safe_strerror(errno));
  ...
  for (ml = mlists; ml->list; ml++)
    {
      ...
      printf("Memory utilization in module %s:", ml->name);
      for (m = ml->list; m->index >= 0; m++)
    if (m->index && mstat[m->index].alloc)
      printf("  %-30s: %10ld", m->format, mstat[m->index].alloc);
    }
}
