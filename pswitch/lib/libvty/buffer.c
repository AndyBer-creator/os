             ...
             int erase_flag, int no_more_flag)
{
  ...
  if (height < 1)
    {
      printf("%s called with non-positive window height %d, forcing to 1",
              __func__, height);
      height = 1;
    }
  else if (height >= 2)
  ...
  if (width < 1)
    {
      printf("%s called with non-positive window width %d, forcing to 1",
              __func__, width);
      width = 1;
    }
  ...
  for (data = b->head; data && (height > 0); data = data->next)
    ...
    /* This should not ordinarily happen. */
      ...
      if (iov != small_iov)
        {
          printf("%s: growing iov array to %d; "
      ...
      else
        {
          ...
          printf("%s: corruption detected: iov_small overflowed; "
     ...
     example: Solaris2.6 are defined IOV_MAX size at 16.     */
  {
    ...
    while (iov_index > 0)
     ...
     if ((nbytes = writev(fd, c_iov, iov_size)) < 0)
       {
         printf("%s: writev to fd %d failed: %s",
  ...
  while (written > 0)
    {
      ...
      if (!(d = b->head))
        {
          printf("%s: corruption detected: buffer queue empty, "
