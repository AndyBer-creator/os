...
int32 cfg_easycwmp_db_init(void)
{
    cfg_easycwmp_t *easycwmp = NULL;

    CFG_OPEN(CFG_ID_EASYCWMP, (cfg_addr)&easycwmp);

    osal_memset(easycwmp, 0, sizeof(cfg_easycwmp_t));

    CFG_CLOSE(CFG_ID_EASYCWMP);
}

int32 cfg_easycwmp_device_get(cfg_device_t *pDeviceInfo)
{
    cfg_easycwmp_t *easycwmp = NULL;

    SYS_PARAM_CHK((NULL == pDeviceInfo), SYS_ERR_NULL_POINTER);

    CFG_OPEN(CFG_ID_EASYCWMP, (cfg_addr)&easycwmp);

    osal_memcpy(pDeviceInfo, &easycwmp->cwmpDevice, sizeof(cfg_device_t));

    CFG_CLOSE(CFG_ID_EASYCWMP);
}

int32 cfg_easycwmp_device_set(cfg_device_t *pDeviceInfo)
{
    cfg_easycwmp_t *easycwmp = NULL;
    ...
    osal_memcpy(&easycwmp->cwmpDevice, pDeviceInfo, sizeof(cfg_device_t));
