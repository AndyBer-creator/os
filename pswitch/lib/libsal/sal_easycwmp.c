...
#include <libuci/uci_config_api.h>

int32 ipc_easycwmp_enable_set(sys_enable_t enable)
{
	int                      iRet;
	SMD_MSG 				 msg;
	ARG_DEF(1);

	smdMsgInit(&msg, SMD_MSG_TYPE_APP, EASYCWMP_FUNC_ENABLE, SMD_GLOBAL, SMD_MSG_SET);
    ARG_ADD(0, &enable, 0, sizeof(sys_enable_t));

    iRet = smdMsgRequestByName(EASYCWMP_SERVER_PATH, &msg, ARG_GET(), 1, VOS_TIMEOUT_DEFAULT);
...
int32 sal_easycwmp_db_init(void)
{
...
int32 sal_easycwmp_swVersion_set(char* ver)
{
	uci_param_t data = {
		.package = "easycwmp",
		.section = "@device[0]",
	...
	SYS_PARAM_CHK((NULL == ver), SYS_ERR_NULL_POINTER);

    osal_strncpy(data.value, ver, osal_strlen(ver)+1);
	SYS_ERR_CHK(uci_setParamValue_api(&data));

    return SYS_ERR_OK;
} 

int32 sal_easycwmp_swVersion_get(char* pVersion)
{
	...
	SYS_ERR_CHK(uci_getParamValue_api(&value));

	osal_strncpy(pVersion, value.value, osal_strlen(value.value)+1);

    return SYS_ERR_OK;
} 

int32 sal_easycwmp_sn_set(char* sn)
{
	SYS_PARAM_CHK((NULL == sn), SYS_ERR_NULL_POINTER);
	...
	osal_strncpy(data.value, sn, osal_strlen(sn)+1);
	SYS_ERR_CHK(uci_setParamValue_api(&data));

    return SYS_ERR_OK;
} 

int32 sal_easycwmp_sn_get(char* sn)
{
	SYS_PARAM_CHK((NULL == sn), SYS_ERR_NULL_POINTER);
	...
	osal_strncpy(sn, value.value, osal_strlen(value.value)+1);

    return SYS_ERR_OK;
} 

int32 sal_easycwmp_device_cfg_get(int index, char* strValue)
{
	SYS_PARAM_CHK((NULL == strValue), SYS_ERR_NULL_POINTER);
	...
	switch(index)
	{
		case EASYCWMP_DEVICE_MANUFACTURER:
			osal_strcpy(data.option, "manufacturer");
			break;
		case EASYCWMP_DEVICE_OUI:
			osal_strcpy(data.option, "oui");
			break;
		case EASYCWMP_DEVICE_PRODUCT_CLASS:
			osal_strcpy(data.option, "product_class");
			break;
		case EASYCWMP_DEVICE_SN:
			osal_strcpy(data.option, "serial_number");
			break;
		case EASYCWMP_DEVICE_SOFTWAREVERSION:
			osal_strcpy(data.option, "SoftwareVersion");
			break;
		case EASYCWMP_DEVICE_HARDWAREVERSION:
			osal_strcpy(data.option, "HardwareVersion");
			break;
		case EASYCWMP_DEVICE_MODENAME:
			osal_strcpy(data.option, "modelName");
			break;
	...
	SYS_ERR_CHK(uci_getParamValue_api(&data));

	osal_strncpy(strValue, data.value, osal_strlen(data.value)+1);

    return SYS_ERR_OK;
} 

int32 sal_easycwmp_acs_cfg_get(int index, char* strValue)
{
	...
	switch(index)
	{
		case EASYCWMP_ACS_URL:
			osal_strcpy(data.option, "url");
			break;
		case EASYCWMP_ACS_USERNAME:
			osal_strcpy(data.option, "username");
			break;
		case EASYCWMP_ACS_PASSWORD:
			osal_strcpy(data.option, "password");
			break;
		case EASYCWMP_ACS_PERIODIC_ENABLE:
			osal_strcpy(data.option, "periodic_enable");
			break;
		case EASYCWMP_ACS_PERIODIC_INTERVAL:
			osal_strcpy(data.option, "periodic_interval");
			break;
		case EASYCWMP_ACS_PERIODIC_TIME:
			osal_strcpy(data.option, "periodic_time");
			break;
...
int32 sal_easycwmp_acs_cfg_set(int index, char* strValue)
{
	SYS_PARAM_CHK((NULL == strValue), SYS_ERR_NULL_POINTER);
	...
	osal_strncpy(data.value, strValue, osal_strlen(strValue)+1);
	SYS_ERR_CHK(uci_setParamValue_api(&data));

    return SYS_ERR_OK;
} 

int32 sal_easycwmp_local_cfg_get(int index, char* strValue)
{
	SYS_PARAM_CHK((NULL == strValue), SYS_ERR_NULL_POINTER);
	...
	switch(index)
	{
		case EASYCWMP_LOCAL_ENABLE:
			osal_strcpy(data.option, "enable");
			break;
		case EASYCWMP_LOCAL_INTERFACE:
			osal_strcpy(data.option, "interface");
			break;
		case EASYCWMP_LOCAL_PORT:
			osal_strcpy(data.option, "port");
			break;
		case EASYCWMP_LOCAL_USERNAME:
			osal_strcpy(data.option, "username");
			break;
		case EASYCWMP_LOCAL_PASSWORD:
			osal_strcpy(data.option, "password");
			break;
		case EASYCWMP_LOCAL_AUTH:
			osal_strcpy(data.option, "authentication");
			break;
		case EASYCWMP_LOCAL_LOG_LEVEL:
			osal_strcpy(data.option, "logging_level");
			break;
...
int32 sal_easycwmp_local_cfg_set(int index, char* strValue)
{
	SYS_PARAM_CHK((NULL == strValue), SYS_ERR_NULL_POINTER);
...
int32 sal_easycwmp_cpe_enable_set(sys_enable_t enable)
{
	...
	ipc_easycwmp_enable_set(enable);
    return SYS_ERR_OK;
}

int32 sal_easycwmp_modelName_set(char* model)
{
	...
	SYS_PARAM_CHK((NULL == model), SYS_ERR_NULL_POINTER);

	osal_strncpy(data.value, model, osal_strlen(model)+1);
	SYS_ERR_CHK(uci_setParamValue_api(&data));
...
int32 ipc_dhcp_server_IpBind_Switch_enable_set(sys_enable_t enable)
{
    ...
    if ( VOS_ERR_NONE != iRet )
    {
        osal_printf("%s:request failed iRet = %d\n", __func__, iRet);
    }
...
int32 ipc_dhcp_client_host_mac_ip_del(sys_mac_t* pMac, uint16 vid)
{
    ...
    if ( VOS_ERR_NONE != iRet )
    {
        osal_printf("%s:request failed iRet = %d", __func__, iRet);
    }
...
int ipc_dhcp_client_ip_check_get(uint32 ipaddr, uint32* pUserd)
{
	...
	if ( VOS_ERR_NONE != iRet )
	{
        osal_printf("\r\n %s %d cmd send fail", __func__, __LINE__);
	}
...
int ipc_ipPoolDisableIp(BOOL bDisable, char *szPoolName, sys_ipv4_t ulIp, int *piErrNo)
{
    ...
    if ( VOS_ERR_NONE != iRet )
    {
        osal_printf("\r\n cmd send ipc_ipPoolDisableIp fail");
    }
...
int32 ipc_dhcp_server_check_group_used(uint32 group, uint32* pUsed, uint32* vid)
{
    ...
    if ( VOS_ERR_NONE != iRet )
    {
        osal_printf("\r\n ipc_dhcp_server_check_group_used");
    }
