...
#define tac_abort(e, str) { int _e; _e = e;\
       _e = errno; session->aborted = 1; tac_close(session); \
       if((str)) { \
...
/* if timeout */
static struct session* catch_sess = NULL;
static void catchup(int s) {
   //tac_error("*** TACACS+ Server Not Responding!\r\n");
    catch_sess->aborted = 1;
    tac_close(catch_sess);
    /* RTK :*/
    catch_sess = NULL;
}
...
tac_connect(const char *peer, int timeout, const char *key, int port) {
  ...
  /* connection */
#if 1 /* RTK: Support IPv4, IPv6, Hostname */
    if (inet_pton(AF_INET, peer, &s.sin.sin_addr))
    {
        ...
        if (bind(session->sock, (struct sockaddr *)&s.sin, sizeof(s.sin)) < 0)
            tac_abort(errno,"bind");

        s.sin.sin_family = AF_INET;
        s.sin.sin_port = htons(port);
        inet_pton(AF_INET, peer, &s.sin.sin_addr);
    }
    else if (inet_pton(AF_INET6, peer, &s.sin6.sin6_addr))
    {
        if ((session->sock = socket(AF_INET6, SOCK_STREAM, 0)) < 0)
            tac_abort(errno,"socket");
        ...
        if (bind(session->sock, (struct sockaddr *)&s.sin6, sizeof(s.sin6)) < 0)
            tac_abort(errno,"bind");

        s.sin6.sin6_family = AF_INET6;
        s.sin6.sin6_port = htons(port);
        inet_pton(AF_INET6, peer, &s.sin6.sin6_addr);
    }
    else
    {
        ...
        if (getaddrinfo(peer, NULL, &hints, &result) < 0)
            tac_abort(errno,"host not found");

        for (rp = result; rp != NULL; rp = rp->ai_next) {
            session->sock = socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol);
        ...
        if (rp == NULL) /* No address succeeded */
            tac_abort(errno,"Could not bind");

        if (AF_INET == rp->ai_family)
        {
            memcpy(&s.sin, rp->ai_addr, sizeof(s.sin));
        ...
        {
            memcpy(&s.sin6, rp->ai_addr, sizeof(s.sin6));
            s.sin6.sin6_family = AF_INET6;
            s.sin6.sin6_port = htons(port);
        }
...
/*  if (!inet_aton(session->peer, &s.sin_addr)) tac_abort; */
  ...
  /* RTK: (START) Make libtacplus allow host name*/
  if (!inet_aton(peer, &s.sin.sin_addr))
  {
    struct hostent *hent = NULL;
    if ((hent = gethostbyname(peer)) == NULL)
    {
      tac_abort(errno, "host not found");
    }
    memcpy(&s.sin.sin_addr, hent->h_addr, sizeof(s.sin.sin_addr));
  }
  /* RTK: (END)*/
#ifndef __SVR4
#ifndef __linux__
  ...
  if (connect(session->sock, (struct sockaddr *)&s, sizeof(s)) < 0) {
    ...
    /* RTK: (START) Allow server timeout from 1~xxxx*/
    while (e == EHOSTUNREACH) /* No Route to Host */
    {
        if (catch_sess == NULL)
            return NULL;
        if (connect(session->sock, (struct sockaddr *)&s, sizeof(s)) < 0)
