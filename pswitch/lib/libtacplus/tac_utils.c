...
/*
/////////////////////////////////////////////
// сверяе?пароль, полученный от NAS??пароле?// полученным из базы. Если не проходит прямое сравнени?// делаем сравнени?по MD5 digest (дл?CHAP)
//
...
compare_password(char* pwduser, char* pwdnas) {
    ...
    mdp[0] = id;
    bcopy(secret, &mdp[1], strlen(pwduser));
    chal = pwdnas + 1;
