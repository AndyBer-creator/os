...
if(isIE() && isIE() < 9){
  var postop = (IsUndefOrNull($('#main_table').position()) ? 0 : $('#main_table').position().top);
  var position = (0 - postop + 15);
  if (!IsUndefOrNull(isDialogOpen) && isDialogOpen)
    j.css({top:position+ $('#main_table').height()});
  else
    j.css({top:position});
