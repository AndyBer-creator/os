...
static int32 cgi_rip_prop_get(void);

const cgi_cmdmap_t cmdGetRipTbl[] =
{
    {CGI_CMD_RIP_CFG,               cgi_rip_prop_get},
...
static int32 cgi_rip_prop_get(void)
{
    ...
    while (1)
    {
        ...
        IPV42STR(ripEnableNwInfo.ulDestMask, ripNetworkMask);
        cJSON_AddStringToObject(network, CGI_RIP_NETWORKADDR, ripNetworkIp);
        cJSON_AddStringToObject(network, CGI_RIP_NETWORKMASK, ripNetworkMask);
    }
