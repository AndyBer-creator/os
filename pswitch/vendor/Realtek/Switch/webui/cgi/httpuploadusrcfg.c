...
int main(int argc, char* argv[], char* envp[])
{
    ...
    if ((fp = fopen(CONF_HTTP_USRCFG_TMPFILE, "w")) == NULL)
    {
    ...
    else
    {
        ...
        if (NULL != (fp = fopen(CONF_HTTP_USRCFG_TMPFILE_STATUS, "w")))
        {
            fprintf(fp, "%d", ret);
            fclose(fp);
        }
    ...
    if(ret == SYS_ERR_OK)
    {
    	CGI_PRINTF("</body>User default config update OK, It will redirect in 2 seconds!</html>");
    }
    else
    {
		CGI_PRINTF("</body>User default config update Failed, It will redirect in 2 seconds!</html>");
    }
