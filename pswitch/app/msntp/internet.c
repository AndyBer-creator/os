...
/* Look up the Internet name or IP number. */
        /*  RTK:4633. inet_pton() returns 1 on success.
            It returns -1 if there was an error (errno is set),
            or 0 if the input isn't a valid IP address. */
        struct in_addr tmp;

        //if (! isdigit(hostname[0])) {
        if (1 != inet_pton(AF_INET, hostname , &tmp)) {
            errno = 0;
            host = gethostbyname(hostname);
#if 1 /* Allow IP address w/o host */
            clear_alarm();
...
#if 0 /* Allow IP address w/o host */
            errno = 0;
