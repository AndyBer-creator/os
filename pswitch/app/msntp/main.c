...
#if 0 /* We don't need to ask user agreement */
    if (absoff > prompt) {
    ...
    if (initial) {
        ...
        if (verbose > 2)
            fprintf(stderr,"prev=%.6f when=%.6f retry=%d\n",
                previous,when,retry);
        for (i = 0; i < nhosts; ++i) open_socket(i,hostnames[i],delay, port);
...
triggered if the signal handling works. */
    ...
    for (k = 0; k < nhosts; ++k) open_socket(k,hostnames[k],delay, port);
    if (action != action_display) {
        set_lock(1);
        locked = 1;
    }
...
mainly out of paranoia. */
    ...
    else
    {
        ...
        while (accepts < count && attempts < 2*count)
        {
            if (current_time(JAN_1970) > deadline)
            {
                fp = fopen("/tmp/msntp_reason", "w");
                fprintf(fp, "Request timed out");
                fclose(fp);
            ...
            if (read_packet(cycle,&data,&x,&y))
            {
                ...
                {
                    fp = fopen("/tmp/msntp_reason", "w");
                    fprintf(fp, "Server Unsyncronized");
                    fclose(fp);
                    fatal(0,"Server Unsyncronized",NULL);
                }

                if (++rejects > count)
                {
                    fp = fopen("/tmp/msntp_reason", "w");
                    fprintf(fp, "Bad data encoded");
                    fclose(fp);
                    fatal(0,"too many bad or lost packets",NULL);
                }
                else
...
/* Decode the arguments. */
    ...
    while (argc > 1) {
        ...
        if (strcmp(argv[1],"-B") == 0 && action == 0) {
            ...
            /* RTK: Allow to change port */
            if (sscanf(argv[2],"%d",&port) != 1) syntax(1);
            k = 2;
        } else
            break;
        argc -= k;
