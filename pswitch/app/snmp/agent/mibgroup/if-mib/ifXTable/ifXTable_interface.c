                       ...
                       netsnmp_variable_list * var, int column)
{
    ...
    case COLUMN_IFALIAS:
        ...
        if (SNMPERR_SUCCESS != rc) {
            ...
            char portDesc[CAPA_PORTDESC_STR_LEN+1];
            osal_memset(portDesc,0,sizeof(portDesc));
            osal_memcpy(portDesc, (char *) var->val.string, var->val_len);

            rc = ifAlias_check_value(rowreq_ctx, portDesc,
                                     osal_strlen(portDesc));
            if ((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc)
                && (MFD_NOT_VALID_NOW != rc)) {
                     ...
                     netsnmp_variable_list * var, int column)
{
    ...
    case COLUMN_IFALIAS:
        ...
        rowreq_ctx->column_set_flags |= COLUMN_IFALIAS_FLAG;
        rc = ifAlias_set(rowreq_ctx, portDesc,
                         osal_strlen(portDesc));
