...
/* RTK-20100625: <START> interfaces MIB */
/* Rewrite from netsnmp_arch_interface_container_load() in interface_linux.c
 *
 * @retval  0 success
 ...
 */
int
_snmp_port_interface_container_load(netsnmp_container* container,
                                      u_int load_flags)
{
    netsnmp_interface_entry *entry = NULL;
    sys_logic_port_t logicPort;
    //sys_logic_portmask_t allPmsk;

    DEBUGMSGTL(("[RTK]", "Set physical ports to ifTable. Load (flags %p)\n",
                load_flags));

    if (NULL == container) {
        DEBUGMSGTL(("[RTK]", "no container specified/found for interface\n"));
        snmp_log(LOG_ERR, "no container specified/found for interface\n");
        return -1;
    }


    /* RTK-20100709: get port. logicPort is 0-based */
    FOR_EACH_LOGIC_PORT(logicPort)
    {
        ...
        /* RTK-20100709: logicPort is 0-based. Interface table is 1-based */
        if_index = LP2UP(logicPort);
        LP2FULLSTR(logicPort, name);

         entry = netsnmp_access_interface_entry_create(name, if_index);
        if (NULL == entry) {
            DEBUGMSGTL(("[RTK]", "could not create entry\n"));
            netsnmp_access_interface_container_free(container,
                                                    NETSNMP_ACCESS_INTERFACE_FREE_NOFLAGS);
           return -3;
        }
        entry->ns_flags = flags; /* initial flags; we'll set more later */
        entry->os_flags = flags; /* initial flags; we'll set more later */

        //sal_sys_macAddr_get(entry->paddr);
    ...
    return 0;
} /* RTK-20100625: <END> interfaces MIB */
...
netsnmp_access_interface_container_load(netsnmp_container* container, u_int load_flags)
{
    ...
    /* RTK-20100625: lo and eth0 to ifTable */
    /*    rc =  netsnmp_arch_interface_container_load(container, load_flags); */

    /* RTK-20100625: set physical ports to ifTable */

    // rc =  netsnmp_arch_interface_container_load(container, load_flags);
    rc = _snmp_port_interface_container_load(container, load_flags);
    if (0 != rc) {
        netsnmp_access_interface_container_free(container,
                                                NETSNMP_ACCESS_INTERFACE_FREE_NOFLAGS);
        container = NULL;
...
Interface_Scan_Init(void)
{
    ...
    //add by rtk for compile
    oid       ifTable_oid[] = { 1,3,6,1,2,1,2,2};
