    ...
    *var_len = sizeof(long_ret);        /* assume an integer and change later if not */
    ...
    if (vp->magic == SNMPENABLEAUTHENTRAPS) {
        *write_method = write_snmp;
        ...
        #if 0
        if (SNMPOUTPKTS == vp->magic  || SNMPOUTTRAPS == vp->magic)
        {
            sal_snmp_statistic_get(STAT_SNMPOUTTRAPS, &cnt);
            long_ret = long_ret + cnt;
        }
        #endif
           ...
           size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    case COMMIT:
        ...
        if (1 == intval)
            enable = ENABLED;
#ifdef CONFIG_SYS_MIB_TRAP
        if (SYS_ERR_OK == sal_snmp_trapEnable_set(SNMP_TRAP_CLASS_AUTHENTICATE, enable))
        {
            snmp_enableauthentraps = intval;
        }
        else
