...
vacm_parse_view(const char *token, char *param)
{
    ...
    if (!snmp_parse_oid(subtree, suboid, &suboid_len)) {
     ...
     //RTK change 2013/01/22 for view mask issue
     #if 0
     if (mask) {
        unsigned int val;
        i = 0;
    ...
    if (mask) {
        ...
        for (mask = strtok_r(mask, " .:", &st); mask; mask = strtok_r(NULL, " .:", &st)) {
            ...
            memset(maskHexStr,0, sizeof(maskHexStr));
            sprintf(maskHexStr, "%x", val);
            len = strlen(maskHexStr);
            for (j = len/2 - 1; j >= 0; j--)
            {
                viewMask[i++] = (val >> (j * 8)) & 0xff;
            }
        }
        mask_len = i;
    } else {
                   ...
                   int parsetype, int viewtypes)
{
    ...
    if (cp && *cp) {
        ...
        strcpy(theoid, ".1");
        //jocky modify DFLT_SNMP_VIEW_STR
    ...
    if ((viewtypes == VACM_VIEW_READ_BIT) ||
         ...
         */
        snprintf(line, sizeof(line),
                 "\"%s\" %s %s %s %s \"%s\" \"%s\" \"%s\"",    //jocky deng modify 2015/01/26
