...
#  define MAX_historyControlBucketsRequested           RMON_HISTORY_MAX_BUCKET
#  define MIN_historyControlInterval                       1
//#  define MAX_historyControlInterval                       3600
#  define MAX_historyControlInterval                       RMON_HISTORY_MAX_INTERVAL
...
history_get_backet(unsigned int clientreg, void *clientarg)
{
    ...
    u_long speed = 0;
    uint32  interVal = 0;
    sys_port_linkStatus_t linkStat = PORT_LINKDOWN;
    ...
    newSample.ifIndex = body->data_source.objid[body->data_source.length - 1];
    SYSTEM_get_eth_statistics(&newSample);
   ...
   SYS_MEM_CLEAR(speedDuplex);

   sal_port_linkStatus_get(UP2LP(newSample.ifIndex), &linkStat);
   if (PORT_LINKUP == linkStat)
   {
        if (SYS_ERR_OK == sal_port_speedDuplex_get(UP2LP(newSample.ifIndex), &speedDuplex))
        {
           switch(speedDuplex.speed)
           {
            ...
            interVal = (body->previous_bucket.start_interval/100 - bptr->start_interval/100);
            if (interVal < 1)
                interVal = 1;
            bptr->utilization = (bptr->EthData.octets)/interVal;
            speed /= 10000;
            bptr->utilization = (bptr->utilization * 8)/speed;
        }
        else
        {
            bptr->utilization = 0;
...
history_Activate(RMON_ENTRY_T * eptr)
{
    ...
    body->previous_bucket.EthData.ifIndex = body->data_source.objid[body->data_source.length - 1];
    SYSTEM_get_eth_statistics(&body->previous_bucket.EthData);
                      ...
                      size_t * var_len, WriteMethod ** write_method)
{
    ...
    *var_len = sizeof(long);    /* default */
    ...
    case DATA_DROPEVENTS:
        long_ret = 0;
        return (unsigned char *) &theBucket.EthData.dropEvents;
    case DATA_OCTETS:
        long_ret = 0;
...
init_history(void)
{
    ...
    ROWAPI_init_table(&HistoryCtrlTable, "History", CAPA_RMON_HISTORY_NUM, &history_Create, NULL,   /* &history_Clone, */
                      NULL,     /* &history_Delete, */
                      &history_Validate,
                      &history_Activate,
