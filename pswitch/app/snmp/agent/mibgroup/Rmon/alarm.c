...
/* RTK. For syslog */
const char text_rmon_samples[][24] =
{
/* SAMPLE_NOUSE0 */          "NOUSE",
/* SAMPLE_NOUSE1 */          "NOUSE",
/* SAMPLE_NOUSE2 */          "NOUSE",
/* SAMPLE_DROPEVENTS */      "DropEvents",
/* SAMPLE_OCTETS */          "Octets",
/* SAMPLE_PKTS */            "Pkts",
/* SAMPLE_BROADCASTPKTS */   "BroadcastPkts",
/* SAMPLE_MULTICASTPKTS */   "MulticastPkts",
/* SAMPLE_CRCALIGNERRORS */  "CRCAlignErrors",
/* SAMPLE_UNDERSIZEPKTS */   "UnderSizePkts",
/* SAMPLE_OVERSIZEPKTS */    "OverSizePkts",
/* SAMPLE_FRAGMENTS */       "Fragments",
/* SAMPLE_JABBERS */         "Jabbers",
/* SAMPLE_COLLISIONS */      "Collisions",
/* SAMPLE_PKTS64OCTETS */    "Pkts64Octets",
/* SAMPLE_PKTS65TO127OCTETS */   "Pkts65to127Octets",
/* SAMPLE_PKTS128TO255OCTETS */  "Pkts128to255Octets",
/* SAMPLE_PKTS256TO511OCTETS */  "Pkts256to511Octets",
/* SAMPLE_PKTS512TO1023OCTETS */ "Pkts512to1023Octets",
/* SAMPLE_PKTS1024TO1518OCTETS */"Pkts1024to1518Octets",
};
...
alarm_check_var(unsigned int clientreg, void *clientarg)
{
    ...
    if (SAMPLE_TYPE_ABSOLUTE == body->sample_type)
        ...
        body->value = (new_value >= body->last_abs_value) ? new_value - body->last_abs_value : new_value;
...
#ifdef CONFIG_SYS_MIB_RMON_MGMT_UI
        /* RTK, syslog */
        SYS_LOG(LOG_CAT_RMON, LOG_RMON_ALARM_RISING,
            body->value, text_rmon_samples[body->var_name.objid[body->var_name.length-2]],
            body->var_name.objid[body->var_name.length-1],
            body->rising_threshold);
#endif
    }
...
#ifdef CONFIG_SYS_MIB_RMON_MGMT_UI
        /* RTK, syslog */
        SYS_LOG(LOG_CAT_RMON, LOG_RMON_ALARM_FALLING,
            body->value, text_rmon_samples[body->var_name.objid[body->var_name.length-2]],
            body->var_name.objid[body->var_name.length-1],
            body->falling_threshold);
#endif
        ...
        if (ALARM_RISING == body->startup_type ||
            ...
            if (body->last_abs_value >= body->rising_threshold) {
                event_api_send_alarm(1, eptr->ctrl_index,
                                     ...
                                     body->var_name.objid,
                                     body->var_name.length,
                                     body->sample_type, body->last_abs_value,
                                     body->rising_threshold,
                                     "Startup Rising");
#ifdef CONFIG_SYS_MIB_RMON_MGMT_UI
                /* RTK, syslog */
                SYS_LOG(LOG_CAT_RMON, LOG_RMON_ALARM_RISING,
                    body->last_abs_value,
                    text_rmon_samples[body->var_name.objid[body->var_name.length-2]],
                    body->var_name.objid[body->var_name.length-1],
                    body->rising_threshold);
#endif
            }
                 ...
                 oid * name, size_t name_len)
{
    ...
    case RESERVE2:
        ...
        case IDalarmInterval:
            ...
            SYS_PARAM_CHK((long_tmp > RMON_ALARM_MAX_INTERVAL || long_tmp < 1), SNMP_ERR_BADVALUE);
            break;
        ...
        case IDalarmRisingThreshold:
            ...
            SYS_PARAM_CHK((long_tmp > RMON_ALARM_MAX_RISING || long_tmp < 0), SNMP_ERR_BADVALUE);
            break;
        case IDalarmFallingThreshold:
            ...
            SYS_PARAM_CHK((long_tmp > RMON_ALARM_MAX_FALL || long_tmp < 0), SNMP_ERR_BADVALUE);
            break;
        case IDalarmRisingEventIndex:
            snmp_status = AGUTIL_get_int_value(var_val, var_val_type, var_val_len, 0,   /* min. value */
                                               RMON_ENTRY_INDEX_MAX,       /* max. value */
                                               &long_tmp);
            if (SNMP_ERR_NOERROR != snmp_status) {
                return snmp_status;
            }
            cloned_body->rising_event_index = long_tmp;
            SYS_PARAM_CHK((long_tmp > RMON_ENTRY_INDEX_MAX || long_tmp < 1), SNMP_ERR_BADVALUE);
            break;
...
init_alarm(void)
{
    ...
    ROWAPI_init_table(&AlarmCtrlTable, "Alarm", CAPA_RMON_ALARM_NUM, &alarm_Create, NULL, /* &alarm_Clone, */
                      NULL,     /* &alarm_Delete, */
                      &alarm_Validate,
                      &alarm_Activate, &alarm_Deactivate, &alarm_Copy);
}

void ROWAPI_alarm_interval_get(u_long alarm_index, u_long *pInterval)
{
    RMON_ENTRY_T  *hdr;
    CRTL_ENTRY_T   *body;

    hdr =  ROWAPI_find(table_ptr, alarm_index);
    if (hdr == NULL)
        return ;

     body = (CRTL_ENTRY_T *) hdr->body;

     *pInterval = body->interval;
}
/*
 * end of file alarm.c
 */
