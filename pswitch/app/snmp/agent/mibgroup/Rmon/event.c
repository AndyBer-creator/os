 ...
 * SOFTWARE.
 *
 * 2013/09/13: RTK modified. Change event log message.
 ******************************************************************/
                   ...
                   oid * name, size_t name_len)
{
    ...
    case RESERVE2:
        ...
        case Leaf_event_description:
            ...
            SYS_PARAM_CHK((var_val_len > RMON_ENENT_DESC_MAX_LEN), SNMP_ERR_WRONGLENGTH);
        ...
        case Leaf_event_community:
            ...
            if (!char_temp)
                return SNMP_ERR_TOOBIG;
            SYS_PARAM_CHK((var_val_len > CAPA_SNMP_COMMUNITY_LEN), SNMP_ERR_WRONGLENGTH);
        ...
        case Leaf_eventOwner:
            ...
            SYS_PARAM_CHK((var_val_len > RMON_OWNER_NAME_LEN), SNMP_ERR_WRONGLENGTH);
...
#define UNEQ_LENGTH	(1 + 11 + 4 + 11 + 1 + 20)
    ...
    /* RTK Modify message content */
    sprintf(expl, "MIB Var.: %s , %s , %s , Actual Val: %ld , Thresh.Set: %ld , Interval(sec): %ld",
        pch, (sample_type == 1) ? "Absolute" : "Delta",
        (is_rising) ? "Rising" : "Falling", (unsigned long) value, the_threshold, interval);
...
#endif
extern void            send_enterprise_trap_vars(int trap, int specific,
                ...
                u_int sample_type)
{
    ...
    //sal_snmp_rmonEventTrap_send(is_rising, alarm_index, value, the_threshold, alarmed_var, alarmed_var_length, sample_type);
#if 1 /*Rtk change*/
    ...
    static oid      trap_oid[] = { 1, 3, 6, 1, 6, 3, 1, 1, 4, 1, 0 };     /* trap oid */
    ...
    snmp_varlist_add_variable(&top, trap_oid, OID_LENGTH(trap_oid),
                               ASN_OBJECT_ID, rmon1_trap_oid, sizeof(oid) * OID_LENGTH(rmon1_trap_oid));

    snmp_varlist_add_variable(&top, alarm_index_oid, OID_LENGTH(alarm_index_oid),
                               ASN_INTEGER,&alarm_index, sizeof(u_int));

    snmp_varlist_add_variable(&top, sample_type_oid, OID_LENGTH(sample_type_oid),
                               ASN_INTEGER,&sample_type,sizeof(u_int));

    snmp_varlist_add_variable(&top, value_oid, OID_LENGTH(value_oid),
                               ASN_INTEGER,&value,sizeof(u_int));

    snmp_varlist_add_variable(&top, threshold_oid, OID_LENGTH(threshold_oid),
                               ASN_INTEGER,&the_threshold,sizeof(u_int));

    snmp_varlist_add_variable(&top, alarmed_var_oid, OID_LENGTH(alarmed_var_oid),
                               ASN_OBJECT_ID, alarmed_var, sizeof(oid) *alarmed_var_length);
                     ...
                     u_long value, u_long the_threshold, char *alarm_descr)
{
...
#if 0
    ag_trace("event_api_send_alarm(%d,%d,%d, %d, %d,%d, '%s')",
             (int) is_rising, (int) alarm_index, (int) event_index, sample_type,value,the_threshold,
             alarm_descr);
#endif
    ...
    ROWAPI_alarm_interval_get(alarm_index, &interval);
...
init_event(void)
{
    ...
    ROWAPI_init_table(&EventCtrlTable, "Event", CAPA_RMON_EVENT_NUM, &event_Create, &event_Clone, &event_Delete, NULL,    /* &event_Validate, */
