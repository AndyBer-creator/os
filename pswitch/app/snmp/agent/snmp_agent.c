                   ...
                   netsnmp_pdu *pdu, void *magic)
{
    ...
    if (pdu->version  == SNMP_VERSION_3 &&
        session->s_snmp_errno == SNMPERR_USM_AUTHENTICATIONFAILURE) {
                /*jocky modify 2010/09/26*/
//           send_easy_trap(SNMP_TRAP_AUTHFAIL, 0);

            if (pdu->securityNameLen <= 128)
            {
                memcpy(secName,pdu->securityName,pdu->securityNameLen);
            }
            send_authenticationFailure_trap(secName);
            return 1;
    }
    ...
    if ((access_ret = check_access(asp->pdu)) != 0) {
        if (access_ret == VACM_NOSUCHCONTEXT) {
             ...
             */
            /*jocky deng modify 2010/09/26*/
             if (pdu->community_len <= 128)
                memcpy(secName,pdu->community,pdu->community_len);
...
#else
    static u_long sysuptime;
    osal_time_sysUptime_get(&sysuptime);
    return sysuptime/10;    /*the time unit in SNMP is 10ms*/
#endif
}
