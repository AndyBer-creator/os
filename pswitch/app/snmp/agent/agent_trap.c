...
convert_v1pdu_to_v2( netsnmp_pdu* template_v1pdu )
{
    ...
    oid                     snmptrapenterprise[CAPA_SNMP_VIEW_SUBTREE_LEN];
...
{
    oid enterTrapOid[CAPA_SNMP_VIEW_SUBTREE_LEN];
    fds_sys_objid_array_get(enterTrapOid);

    #if 0 /*RTK change, org*/
    if (trap == SNMP_TRAP_ENTERPRISESPECIFIC)
    {
        send_enterprise_trap_vars(trap, specific, enterTrapOid,
                                  SYS_OBJECT_ID_LEN, vars);
        #if 0
