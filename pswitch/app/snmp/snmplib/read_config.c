...
#define CONFIG_MAX_RECURSE_DEPTH 16
    ...
    while (fgets(line, sizeof(line), ifile) != NULL) {
        ...
        if ((cptr = skip_white(cptr))) {
            ...
            if (cptr == NULL) {
              //RTK change 2013/0520 for Error:Blank line following oldEngineID token
		//netsnmp_config_error("Blank line following %s token.", token);
		;
            } else {
...
read_config_store(const char *type, const char *line)
{
    ...
    //RTK change 2013/0704, not check snmpapp.conf
   if (0 == memcmp(type, "snmpapp",7))
       return;
