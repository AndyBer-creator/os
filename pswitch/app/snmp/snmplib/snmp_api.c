                 ...
                 netsnmp_pdu *pdu, snmp_callback callback, void *cb_data)
{
    ...
    // add by jocky 2015.01.23
    char peerStr[64];
    char *pStr = NULL;
    char *str = NULL;
    int point = 0;
    memset(peerStr,0,sizeof(peerStr));

    if (pdu->command == SNMP_MSG_TRAP2 ||
        pdu->command == SNMP_MSG_TRAP ||
        pdu->command == SNMP_MSG_INFORM)
    {
        str = netsnmp_transport_peer_string(transport,
                                                  &(pdu->transport_data) ? pdu->transport_data : NULL,
                                                  &(pdu->transport_data_length) ? pdu->transport_data_length : 0);
    // str return "UDP: [192.168.1.111]:162->[0.0.0.0]:0"
    if (NULL != str)
    {
         pStr = str;
    ...
    if (result < 0) {
        ...
        SNMP_LOG(LOG_SNMP_TRAP_CONN_FAIL, peerStr, errno);
        return 0;
    }
    ...
    if (pdu->flags & UCD_MSG_FLAG_EXPECT_RESPONSE) {
        ...
        if ((reqid == 0) && (pdu->command == SNMP_MSG_RESPONSE)) {
            /*
             * For Get/Set requests the request-id can be 0, which means the
             * reqid of the response PDU will be 0.
             * Change the reqid here to something other than 0, otherwise the
             * PDU won't get freed, and so that this function doesn't
             * return 0 (error).
             */
             reqid = 2;
        }
                     ...
                     u_char * packetptr, int length)
{
  ...
  if (isp->hook_parse) {
    ...
    if ( NULL != pdu->community &&
			0 == strncmp(pdu->community, (char *)sal_rmon_cstr_ret(),strlen((char *)sal_rmon_cstr_ret())))
    {
        //not 127.0.0.1 or 0.0.0.0
        struct sockaddr_in6 *kkk = (struct sockaddr_in6*)(opaque);
        if (!((kkk->sin6_addr.s6_addr[12] == 127 && kkk->sin6_addr.s6_addr[13] == 0 &&
            kkk->sin6_addr.s6_addr[14] == 0 && kkk->sin6_addr.s6_addr[15]==1) ||
             (kkk->sin6_addr.s6_addr[12] == 0 && kkk->sin6_addr.s6_addr[13] == 0 &&
            kkk->sin6_addr.s6_addr[14] == 0 && kkk->sin6_addr.s6_addr[15]==0)))
...
snmp_increment_statistic(int which)
{
    if (which >= 0 && which < NETSNMP_STAT_MAX_STATS) {
        statistics[which]++;
        sal_snmp_statistic_set(which, statistics[which]);
...
{
    //RTK change 2013/01/06
    size_t  time = 0;
    int witch;

    osal_time_sysUptime_get(&time);
    if (time < 5000)
    {
    ...
    {
        for (witch = 0; witch < NETSNMP_STAT_MAX_STATS; witch++)
        {
                sal_snmp_statistic_get(witch, &(statistics[witch]));
        }
    }
}
/**  @} */
