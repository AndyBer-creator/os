 ...
 */
 void str2EngineId(char *strEngine, char *octetStr, int *len)
{
    int iLen, i;
    char *readfrom = strEngine;
    char *cptr = octetStr;
    u_int           tmp;

    if (strEngine == NULL || octetStr == NULL)
        return ;

    iLen = osal_strlen(strEngine);
...
{
    int i;
    char octetStr[BOND_SNMP_ENGINEID_LEN_MAX/2+1];
    osal_memset(octetStr,0,sizeof(octetStr));
    int len;


    str2EngineId(cptr,octetStr,&len);

    if (oldEngineID == NULL)
    {
        if ((oldEngineID = (u_char *) malloc(len + 1)) == NULL)
        {
                return NULL;
        }
    }

    oldEngineIDLength = len;
    osal_memcpy(oldEngineID,octetStr,oldEngineIDLength);
...
snmpv3_store(int majorID, int minorID, void *serverarg, void *clientarg)
{
  ...
  memset(c_engineID, 0, sizeof(c_engineID));
...
extern const char *gsecLevel[];
    ...
    for(index = 0; index < CAPA_SNMP_USER_NUM; index++)
    {
     ...
     if(0 != strlen(user.userName))
     {
        if (SNMP_SECLEVEL_NOAUTHNOPRIV == user.selLevel)
         sprintf(line, "createUser \"%s\" ", user.userName);
        else if (SNMP_SECLEVEL_AUTHNOPRIV == user.selLevel)
        {
            sprintf(line, "createUser \"%s\" %s \"%s\" ",
                  user.userName,
                (user.auth_protp != SNMP_SSL_TYPE_NONE) ? snmpv3_ssl_type[user.auth_protp] : "",
                  user.authKey);
        }
        else
        {
         sprintf(line, "createUser \"%s\" %s \"%s\" %s  \"%s\" ",
                  user.userName,
                (user.auth_protp != SNMP_SSL_TYPE_NONE) ? snmpv3_ssl_type[user.auth_protp] : "",
                  user.authKey,
                (user.encrypt_proto != SNMP_SSL_TYPE_NONE) ? snmpv3_ssl_type[user.encrypt_proto] : "",
                 user.privKey);
         }
        read_config_store(type, line);

        sprintf(line,  "%s \"%s\" %s \n",
                (user.access == SNMP_COMMUNITY_ACCESS_READ_ONLY) ? "rouser" : "rwuser",
                user.userName,
    ...
    osal_memcpy(engineID,engineId,osal_strlen(engineId));
    engineIDLength = osal_strlen(engineId);
#endif
    //engineID_conf(NULL,engineId);
}
#endif

   //test
   //sprintf(line, "createUser user1  \n  rouser user1 noauth \n");
   //read_config_store(type, line);
    ...
    if (engineIDLen) {
         ...
         */
      sprintf(line, "oldEngineID %s",  c_engineID);
