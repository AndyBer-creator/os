                  ...
                  int result, netsnmp_pdu *pdu)
{
    ...
    if (SNMPERR_USM_AUTHENTICATIONFAILURE == result ||
        SNMPERR_USM_UNKNOWNSECURITYNAME == result ||
        SNMPERR_USM_DECRYPTIONERROR == result)
        {
            if (SNMPERR_USM_AUTHENTICATIONFAILURE == result)
                SNMP_LOG(LOG_SNMP_USER_REJECT, pStr[0]);
            else if (SNMPERR_USM_UNKNOWNSECURITYNAME == result)
                SNMP_LOG(LOG_SNMP_USER_REJECT, pStr[1]);
            else if (SNMPERR_USM_DECRYPTIONERROR == result)
                SNMP_LOG(LOG_SNMP_USER_REJECT, pStr[2]);
        }
    #endif
