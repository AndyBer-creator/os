 ...
 */
int
netsnmp_get_var_data(unsigned long *val, unsigned long val_len, netsnmp_variable_list * varlist)
{
    #if 0
    snmp_log(LOG_DEBUG,"get: \n");
    snmp_log(LOG_DEBUG,"type:%d, val:%d, %d \n", varlist->type, varlist->val.integer, *varlist->val.integer);
    snmp_log(LOG_DEBUG,"odi:%s\n", varlist->name_loc);
    #endif
    return 0;
}
