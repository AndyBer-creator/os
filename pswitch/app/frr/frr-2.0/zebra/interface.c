...
static int
user_ip_address_install (struct interface *ifp,
		    const char *addr_str, const char *peer_str,
		    const char *label)
{
  struct zebra_if *if_data;
...
static int
user_ip_address_uninstall (struct interface *ifp,
		      const char *addr_str, const char *peer_str,
		      const char *label)
{
  struct prefix_ipv4 cp;
...
/*******************************************************************************
函数名称：zebraIfShutdown
功能描述：ip接口关闭或者开启
输入参数：ifname，ip接口名称
          bShutdown，TRUE为关闭，FALSE为开启
输出参数：无
函数返回：VD_E_NONE, 成功
          VD_E_FAIL，执行失败
其它说明：无
*******************************************************************************/
int zebraIfShutdown(int *ifname, BOOL bShutdown)       
{
    int                       ret;
    struct interface       *ifp;
    struct zebra_if         *if_data;
...
/*******************************************************************************
函数名称：zebraIfIpAddressAdd
功能描述：ip接口添加ip地址
输入参数：ifname，ip接口名称
          addr_str，ip地址字符串，格式为A.B.C.D/len，比如10.0.0.1/8
输出参数：无
函数返回：VD_E_NONE, 成功
          VD_E_FAIL，执行失败
其它说明：无
...
int zebraIfIpAddressAdd(char *ifname, char *addr_str)
{
    ...
    return user_ip_address_install(ifp, addr_str, NULL, NULL);
}

/*******************************************************************************
函数名称：zebraIfIpAddressDelete
功能描述：ip接口删除ip地址
输入参数：ifname，ip接口名称
          addr_str，ip地址字符串，格式为A.B.C.D/len，比如10.0.0.1/8
输出参数：无
...
int zebraIfIpAddressDelete(char *ifname, char *addr_str)
{
    ...
    return user_ip_address_uninstall(ifp, addr_str, NULL, NULL);
}
