...
/*******************************************************************************
函数名称：zebraRouterIdAdd
功能描述：添加路由器ID
输入参数：id_str，路由器ID，格式A.B.C.D
输出参数：无
函数返回：VD_E_NONE, 成功
          VD_E_FAIL，执行失败
其它说明：无
*******************************************************************************/
int zebraRouterIdAdd(char *id_str)
{
  struct prefix rid;
  vrf_id_t vrf_id = VRF_DEFAULT;

  rid.u.prefix4.s_addr = inet_addr (id_str);
...
/*******************************************************************************
函数名称：zebraRouterIdGet
功能描述：获取路由器ID
输入参数：无
输出参数：szRouteId，路由器ID，格式A.B.C.D，如果没有配置，则返回空字符串
函数返回：VD_E_NONE, 成功
          VD_E_FAIL，执行失败
其它说明：无
*******************************************************************************/
int zebraRouterIdGet(char szRouteId[64])
{
    szRouteId[0] = 0;
    struct zebra_vrf *zvrf;
	vrf_id_t vrf_id = VRF_DEFAULT;
    ...
    if (zvrf->rid_user_assigned.u.prefix4.s_addr)
    {
        sprintf(szRouteId, " %s", inet_ntoa(zvrf->rid_user_assigned.u.prefix4));
    }
    return 0;
}
