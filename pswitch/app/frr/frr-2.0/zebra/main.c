...
static int
zebra_smd_read (struct thread *thread)
{
  int iRet = 0;
  int i;
  int nbytes;
  ...
  int zebra_sock = THREAD_FD (thread);

  //while(1)
  {
	  /* Read raw data from socket */
	  iRet = vosMsgRecv(zebra_sock, buf, SMD_READ_BUFSIZ, VOS_TIMEOUT_NOWAIT, &clientAddr);
	  ...
	  else
	  {
		...
		switch ( pSmdMsg->usType )
	    {
	        case SMD_MSG_TYPE_FRR:
	            iRet = zebraFrrWrapProcess(pSmdMsg);
	            break;
		...
		iRet = vosMsgResp(zebra_sock, iRet, pSmdMsg, pSmdMsg->usArgsBufLen + sizeof(SMD_MSG), &clientAddr);
	  }
	}
end:
  thread_add_read (zebrad.master, zebra_smd_read, (void *)zebra_sock, zebra_sock);
  return 0;
}
...
main (int argc, char **argv)
{
  ...
  */
  ...
  /*建立进程间通信IPC*/
  vosMsgInit(ZEBRA_SOCK_PATH, VOS_MSG_SOURCE_PROCESS, &zebraSockFd);
  thread_add_read (zebrad.master, zebra_smd_read, (void *)zebraSockFd, zebraSockFd);

  /*注册处理函数*/
  zebraFrrWrapFunction();
