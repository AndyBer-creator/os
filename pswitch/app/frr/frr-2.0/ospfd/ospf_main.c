...
static int
ospf_smd_read (struct thread *thread)
{
	int iRet = 0;
	...
	int ospf_sock = THREAD_FD (thread);

	/* Read raw data from socket */
	iRet = vosMsgRecv(ospf_sock, buf, SMD_READ_BUFSIZ, VOS_TIMEOUT_NOWAIT, &clientAddr);
	if (iRet < 0)
	{
		  goto end;
	}
	...
	switch ( pSmdMsg->usType )
	{
	    ...
	    case SMD_MSG_TYPE_FRR:
	        iRet = ospfFrrWrapProcess(pSmdMsg);
	        break;
	...
	iRet = vosMsgResp(ospf_sock, iRet, pSmdMsg, pSmdMsg->usArgsBufLen + sizeof(SMD_MSG), &clientAddr);
 
}
end:
  thread_add_read (master, ospf_smd_read, (void *)ospf_sock, ospf_sock);
  return 0;
}
...
#endif
  /*建立进程间通信IPC*/
  vosMsgInit(OSPF_SOCK_PATH, VOS_MSG_SOURCE_PROCESS, &ospfSockFd);
  thread_add_read (master, ospf_smd_read, (void *)ospfSockFd, ospfSockFd);
