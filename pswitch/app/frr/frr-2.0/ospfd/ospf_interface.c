...
int
ospf_default_if_hello_interval(struct interface *ifp)
{
    int type = IF_DEF_PARAMS (ifp)->type;

    if (OSPF_IFTYPE_NBMA == type ||
        OSPF_IFTYPE_POINTOMULTIPOINT == type)
    {
        return OSPF_HELLO_INTERVAL_DEFAULT_MULTI;
    }
    else
    {
...
int
ospf_default_if_dead_interval(struct interface *ifp)
{
    int type = IF_DEF_PARAMS (ifp)->type;
    ...
    if (OSPF_IFTYPE_NBMA == type ||
        OSPF_IFTYPE_POINTOMULTIPOINT == type)
    {
        return OSPF_ROUTER_DEAD_INTERVAL_DEFAULT_MULTI;
    }
    else
    {
        return OSPF_ROUTER_DEAD_INTERVAL_DEFAULT;
    }
}
