...
{
	enum distribute_type dbType;
	
	if (strncmp (type, "i", 1) == 0)
	    dbType = DISTRIBUTE_IN;
	else if (strncmp (type, "o", 1) == 0)
		dbType = DISTRIBUTE_OUT;
	else
	{
		return -1;
	}
	if(isNoFlag)
	{
		distribute_list_set (ifname, dbType, alist_name);
	}
	else
	{
  		distribute_list_unset (ifname, dbType, alist_name);
	}
	return 0;
}
...
int frr_distribute_info_get(DATA_OP_ENT op,  DISTRIBUTE_INFO_T *pDistribute)
{
    ...
    if (DATA_OP_GET == op)
    {
        if ('\0' == pDistribute->ifname[0])
        {
            dist = distribute_lookup(NULL);
        }
        else
        {
            dist = distribute_lookup(pDistribute->ifname);
        }
    ...
    {
        if (('\0' == pDistribute->ifname[0]) && (0 == pDistribute->index))
        {
            /*得到第一个*/
        ...
        else
        {
            ...
            {
                dist = mp->data;
                if (0 == strcmp(dist->ifname, pDistribute->ifname))
                    break;
            }
            if (NULL == mp)
            {
    ...
    {
        pDistribute->index = i;
        strcpy(pDistribute->ifname, dist->ifname);
    }
    if (dist->list[DISTRIBUTE_IN])
    {
        strcpy(pDistribute->list[DISTRIBUTE_IN], dist->list[DISTRIBUTE_IN]);
    }
    else
    {
        pDistribute->list[DISTRIBUTE_IN][0] = '\0'; 
    }
    
    if (dist->list[DISTRIBUTE_OUT])
    {
        strcpy(pDistribute->list[DISTRIBUTE_OUT], dist->list[DISTRIBUTE_OUT]);
    }
    else
    {
        pDistribute->list[DISTRIBUTE_OUT][0] = '\0';
    }
    
    if (dist->prefix[DISTRIBUTE_IN])
    {
        strcpy(pDistribute->prefix[DISTRIBUTE_IN], dist->prefix[DISTRIBUTE_IN]);
    }
    else
    {
        pDistribute->prefix[DISTRIBUTE_IN][0] = '\0';
    }
    
    if (dist->prefix[DISTRIBUTE_OUT])
    {
        strcpy(pDistribute->prefix[DISTRIBUTE_OUT], dist->prefix[DISTRIBUTE_OUT]);
    }
    else
    {
        pDistribute->prefix[DISTRIBUTE_OUT][0] = '\0';
    }
