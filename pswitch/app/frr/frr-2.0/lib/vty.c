...
/* RTK: Keep vty info for switching node by SAL api (Start) */
static struct vty *vty_act = NULL;
/* RTK: Keep vty info for switching node by SAL api (End) */
...
#if 1 /* RTK: Keep vty info for switching node by SAL api (Start) */
    if (NULL == vty_act)
    {
        vty = vty_new ();
        vty->fd = sock;
...
#else /* RTK: Keep vty info for switching node by SAL api (End) */
  vty = vty_new ();
  vty->fd = sock;
  vty->wfd = sock;
  vty->type = VTY_SHELL_SERV;
...
{
#if 0 /* RTK: Keep vty info for switching node by SAL api (Start) */
  int i;
#endif /* RTK: Keep vty info for switching node by SAL api (End) */
