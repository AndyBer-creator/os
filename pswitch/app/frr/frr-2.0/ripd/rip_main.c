...
static int
rip_smd_read (struct thread *thread)
{
  int iRet = 0;
  int i;
  int nbytes;
  ...
  int rip_sock = THREAD_FD (thread);

  //while(1)
  {
	  /* Read raw data from socket */
	  iRet = vosMsgRecv(rip_sock, buf, SMD_READ_BUFSIZ, VOS_TIMEOUT_NOWAIT, &clientAddr);
	  if (iRet < 0)
	  {
	  	  goto end;
	  }
	  else
	  {
		...
		iRet = vosMsgResp(rip_sock, iRet, pSmdMsg, pSmdMsg->usArgsBufLen + sizeof(SMD_MSG), &clientAddr);
	  }
	}
end:
  thread_add_read (master, rip_smd_read, (void *)rip_sock, rip_sock);
  return 0;
}
...
main (int argc, char **argv)
{
 ...
 // vty_read_config (config_file, config_default);
  ...
  //vty_serv_sock (vty_addr, vty_port, vty_sock_path);
  /*建立进程间通信IPC*/
  vosMsgInit(RIP_SOCK_PATH, VOS_MSG_SOURCE_PROCESS, &ripSockFd);
  thread_add_read (master, rip_smd_read, (void *)ripSockFd, ripSockFd);
