...
int frrRipDebugSet(int bIsEnable, char *pRipDebugStr)
{
	if(bIsEnable)
	{
		if (strncmp ("events", pRipDebugStr, strlen (pRipDebugStr)) == 0)
		{
			rip_debug_event = RIP_DEBUG_EVENT;
		}
		else if(strncmp ("packet", pRipDebugStr, strlen (pRipDebugStr)) == 0)
		{
			rip_debug_packet = RIP_DEBUG_PACKET;
			rip_debug_packet |= RIP_DEBUG_SEND;
			rip_debug_packet |= RIP_DEBUG_RECV;
		}
		else if(strncmp ("zebra", pRipDebugStr, strlen (pRipDebugStr)) == 0)
		{
			rip_debug_zebra = RIP_DEBUG_ZEBRA;
		}
		else if(strncmp ("all", pRipDebugStr, strlen (pRipDebugStr)) == 0)
		{
			rip_debug_event = RIP_DEBUG_EVENT;
			rip_debug_packet = RIP_DEBUG_PACKET;
			rip_debug_packet |= RIP_DEBUG_SEND;
