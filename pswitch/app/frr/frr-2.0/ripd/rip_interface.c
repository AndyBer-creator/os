...
int ripNetworkSet(int bNoFlag, u_int32_t ulIp, u_int32_t ulMask, char *ifname)
{
    int ret;
    struct prefix_ipv4 p;
    struct in_addr mask;
	int ripStatus = 0;
 	...
 	ripdFrrStatusGet(&ripStatus);
 	if(!ripStatus)
 		return VOS_RIP_IS_CLOSE;
    ...
    ifc = connected_lookup_prefix(ifp, (struct prefix *) &p);
    if (NULL == ifc)
    {
        return CMD_WARNING;
    }
    if (bNoFlag)
    {
        ret = rip_enable_network_add (ifc);
    }
    else
    {
        ret = rip_enable_network_delete (ifc);
    }
	#endif
    if (bNoFlag)
    {
        ret = rip_enable_network_add (&p);
    }
    else
    {
        ret = rip_enable_network_delete (&p);
    }
    if (ret < 0)
    {
        return CMD_WARNING;//VD_E_EXISTS;
    }
    return CMD_SUCCESS;
}
...
int frrRipEnableNwInfoGet(DATA_OP_ENT op, RIP_ENABLE_NW_INFO_T *pRipEnableNwInfo)
{
    ...
    table = rip_enable_network;
    if (! table)
        return -1;

    ipPrefix.family = AF_INET;
    mask.s_addr = htonl(pRipEnableNwInfo->ulDestMask);
    ipPrefix.prefixlen = ip_masklen(mask);
    ipPrefix.u.prefix4.s_addr = htonl(pRipEnableNwInfo->ulDestIp);
    ...
    {
        if ( (0 == pRipEnableNwInfo->ulDestIp) && (0xffffffff == pRipEnableNwInfo->ulDestMask) )
        {
            rn = route_top(table);
        }
        else
    ...
    {
        iRet = 0;
        pRipEnableNwInfo->ulDestIp = ntohl(rn->p.u.prefix4.s_addr);
        masklen2ip(rn->p.prefixlen, &mask);
        pRipEnableNwInfo->ulDestMask = ntohl(mask.s_addr);
    }
    route_unlock_node(rn);
