...
/* =============== Print HTML Marcos =============== */
#define DHCP_DBG_TARGET_FILE "/tmp/DHCP_DEBUG"
#if 1
#define DHCP_DBG(a,args...) \
do { \
    char tmp_str[256] = ""; \
    char cmd_str[300] = ""; \
    sprintf(tmp_str, "[DHCP_DBG] %s:%d:"#a"", __FUNCTION__,__LINE__, ##args); \
    sprintf(cmd_str, "echo \"%s\" > %s", tmp_str, DHCP_DBG_TARGET_FILE); \
    system(cmd_str); \
} while (0)
#else
#define DHCP_DBG(a,args...) {}
  ...
  do
      ...
      do
	{
	  ...
	  if ( DhcpMsgRecv->xid != xid ) continue;
       /* RTK mantis#3235. Avoid elligal client ip*/
      if (((unsigned char *)&DhcpMsgRecv->yiaddr)[3] == 0) continue;
      if (((unsigned char *)&DhcpMsgRecv->yiaddr)[3] == 255) continue;
...
void (*buildDhcpMsg)(unsigned);
{
    ...
    /* RTK: NGB-47 (Start) */
    {
        ...
        sys_ipv4_t ipv4;
        SYS_MEM_CLEAR(ipv4);
        ipv4 = DhcpIface.ciaddr;
        FOR_EACH_L3_IPV4(i, pCurr)
        {
            if (IS_IPV4_IN_SUBNET(&ipv4, &pCurr->ip))
            {
                sleep(60);
                return &dhcpStop;
            }
...
/* check if the offered IP address already in use */
  ...
  memcpy(DhcpIface.shaddr,UdpIpMsgRecv.ethhdr.ether_shost,ETH_ALEN);
    ...
    /* RTK: Refresh DNS Setting */
    system("handler -d advdns &");
#endif
...
void *dhcpStop()
{
  ...
  p->sin_family = AF_INET;
    ...
    /* RTK : dont set ip to all zero, set static ip from default config */
    char addr[CAPA_IPV4_STR_LEN];

    SYS_MEM_CLEAR(addr);
    osal_memcpy(addr, DFLT_L3_IPV4_ADDR, osal_strlen(DFLT_L3_IPV4_ADDR));
    sys_util_str2Ip(addr, &ip);
    netmask = IPV4_PFX2MASK(DFLT_L3_IPV4_PREFIXLEN);
    SYS_MEM_CLEAR(addr);
    osal_memcpy(addr, DFLT_L3_IPV4_GW, osal_strlen(DFLT_L3_IPV4_GW));
    sys_util_str2Ip(addr, &gw);

  p->sin_addr.s_addr = ip; // 0;
#ifndef OLD_LINUX_VERSION
  if ( ioctl(dhcpSocket,SIOCSIFADDR,&ifr) == -1 )
    syslog(LOG_ERR,"dhcpStop: ioctl SIOCSIFADDR: %m\n");
  memcpy(&p->sin_addr.s_addr,&netmask,4);
