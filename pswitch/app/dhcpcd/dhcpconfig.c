 ...
 */
/* RTK: (20130613) Bonjour refresh with new IP */
/* RTK: (20140605) Refresh DNS setting */
/* RTK: (20161229) Seletct default route by handler */
...
int dhcpConfig()
{
...
#else
	char cmd[128];
	memcpy(&ipv4mask, DhcpOptions.val[subnetMask],4);
	prefix = IPV4_MASK2PFX(ipv4mask);
	sprintf(cmd, "ip addr add %u.%u.%u.%u/%u dev %s", 
  ...
  if ( SetDHCPDefaultRoutes )
    ...
    {
        /* RTK: (20161229) Seletct default route by handler (Start) */
        char _cmd[128];
        char *_gw = DhcpOptions.val[routersOnSubnet];

        memset(_cmd, 0, sizeof(_cmd));

        if (0 == strcmp(ifr.ifr_name, "intfm"))
        {
            system("ip route del default table intfm 1> /dev/null 2> /dev/null");

            sprintf(_cmd, "ip route add default via %u.%u.%u.%u dev %s table intfm 1> /dev/null 2> /dev/null",
                    _gw[0] & 0xFF,
                    _gw[1] & 0xFF,
                    _gw[2] & 0xFF,
                    _gw[3] & 0xFF,
        ...
        sprintf(_cmd, "echo \"%s %u.%u.%u.%u\" > /etc/dhcpc/dfltGw",
                    ifr.ifr_name,
                    _gw[0] & 0xFF,
                    _gw[1] & 0xFF,
                    _gw[2] & 0xFF,
        ...
        system("handler -d dflt_route &");
        /* RTK: (20161229) Seletct default route by handler (End) */
    }
    }
  else
