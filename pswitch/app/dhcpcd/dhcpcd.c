 ...
 *
 * RTK : 2013/08/07 fix optino 57 minimun size less than 576 bug
 * RTK: (20160727) Writ PID file at first
 * RTK: (20161102) New uClibc library should use fork
 */
...
usage:	    print_version();
	    fprintf(stderr,
"Usage: dhcpcd [-dknrbBCDHNRSTY] [-l leasetime] [-h hostname] [-t timeout]\n\
  ...
  /*RTK fix optino57 to minimum 576 */
  if (dhcpMsgSize < 576)
    dhcpMsgSize += (576 - dhcpMsgSize);

  nleaseTime = htonl(LeaseTime);
  alarm(TimeOut);

  /* RTK: (20160727) Writ PID file at first (Start) */
  writePidFile(getpid());
  /* RTK: (20160727) Writ PID file at first (End) */
  do
