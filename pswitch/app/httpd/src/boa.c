...
int main(int argc, char **argv)
{
   ...
   (void) time(&current_time);
    ...
    while ((c = getopt(argc, argv, "c:r:d:sw")) != -1)
    {
        switch (c)
        {
            case 'c':
   ...
   memset(currPidFile, 0, sizeof(currPidFile));
   if ((boa_ssl >= 2 || boa_ssl == 0) && server_port > 0) {
#ifdef CONFIG_SYS_PROTO_WEBAUTH
      if (webauth)
      {
          server_port = WEBAUTH_NAT_TCPPORT_HTTP;
          server_s[0] = create_server_socket(server_port, 0);
          strlcpy(currPidFile, "/var/run/boa_http_wa.pid", sizeof(currPidFile));
      }
      else
#endif
      {
        server_s[0] = create_server_socket(server_port, 0);
        strlcpy(currPidFile, "/var/run/boa.pid", sizeof(currPidFile));
      }
   }
   ...
   if (boa_ssl != 0 && ssl_port > 0) {
      ...
      if (webauth)
      {
          ssl_port = WEBAUTH_NAT_TCPPORT_HTTPS;
          server_s[1] = create_server_socket(ssl_port, 1);
          strlcpy(currPidFile, "/var/run/boa_https_wa.pid", sizeof(currPidFile));
      }
      else
#endif
      {
          server_s[1] = create_server_socket(ssl_port, 1);
          strlcpy(currPidFile, "/var/run/boa_https.pid", sizeof(currPidFile));
      }
   }
   ...
   if (server_s[1].socket == -1 && server_s[0].socket == -1) {
      ...
      /* RTK: SYSLOG */
      SYS_LOG_HANDLE(LOG_CAT_SYSTEM, LOG_SYSTEM_HTTPD_START_FAIL, "");
   ...
   switch (pid) {
	 ...
	 if(currPidFile != NULL) {
	     FILE *PID_FILE = fopen(currPidFile, "w");
	     if(PID_FILE != NULL) {
   ...
   if (maintenance_interval < MIN_MAINTENANCE_INTERVAL) {
      log_error_time();
      fprintf(stderr,
 	 "Changing maintenance mode time interval to %d minutes\n",
...
static server_params *smp_init(socket_type server_s[2])
{
   ...
   if (max_threads > 1) {
      log_error_time();
      fprintf(stderr,
	      "%s: Dispatched %d HTTP server threads.\n", SERVER_NAME,
	      max_threads);
   }
...
static socket_type create_server_socket(int port, int secure)
{
   ...
   /* reuse socket addr */
   if ((setsockopt
	(server_s.socket, SOL_SOCKET, SO_REUSEADDR, (void *) &sock_opt,
	 sizeof(sock_opt))) == -1) {
      DIE("setsockopt");
   }
...
#else
      fputs(SERVER_NAME
	    ": don't know where server root is.  Please #define "
...
static void create_server_names()
{
   ...
   if ( boa_tls_version[0] == 0) {
      strlcpy( boa_tls_version, "Server: "SERVER_NAME"/"SERVER_VERSION" GnuTLS/", sizeof(boa_tls_version));
      strlcat( boa_tls_version, gnutls_check_version(NULL), sizeof(boa_tls_version));
      strlcat( boa_tls_version, "\r\n", sizeof(boa_tls_version));
   }
#endif
}
...
struct rlimit rl;
   ...
   if (rl.rlim_max > rl.rlim_cur) {
...
#endif
    /* RTK: Set resource limit to 8MB (Start)*/
    memset(&rl, 0, sizeof(rl));

    c = getrlimit(RLIMIT_STACK, &rl);
    ...
    rl.rlim_cur = 8388608; /* 8MB */
    c = setrlimit(RLIMIT_STACK, &rl);
    ...
    /* RTK: Set resource limit to 8MB (End)*/
