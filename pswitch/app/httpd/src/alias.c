...
/* RTK: Web Authentication */
typedef struct sys_uri_s
{
    char uri[64];
} sys_uri_t;

const sys_uri_t webauth_uri[] =
{
    { "/cgi/webauth.cgi" },
    { "/css/style.css" },
    { "" }
...
#define WEBAUTH_DFLT_URI webauth_uri[0].uri

static int is_webauth_uri(request * req)
{
    int i = 0;

    while (0 != strlen(webauth_uri[i].uri))
    {
        //fprintf(stderr, "i=%d req->request_uri=%s uri=%s\n", i, req->request_uri, webauth_uri[i].uri);
        if (0 == strcmp(req->request_uri, webauth_uri[i].uri))
            return 1;
   ...
   if (req->user_dir[0] != 0 && req->request_uri[1] == '~') {
	 ...
	 *p = '/';
...
#ifdef CONFIG_SYS_PROTO_WEBAUTH
      /* RTK: Web Authentication */
      if (webauth && !is_webauth_uri(req))
        l2 = strlen(WEBAUTH_DFLT_URI);
      else
	 ...
	 return 0;
      ...
      /* RTK: Web Authentication */
      if (webauth && !is_webauth_uri(req))
        memcpy(buffer + l1, WEBAUTH_DFLT_URI, l2 + 1);
      else
...
static int init_local_cgi_stuff(request * req, int cgi)
{
   ...
   strlcpy(pathname, req->request_uri, sizeof(pathname));
   p = strrchr(pathname, '/');
   if (p)
      *p = 0;
   ...
   strlcpy(pathname, req->document_root, sizeof(pathname));
   strlcat(pathname, req->request_uri, sizeof(pathname));
...
int init_script_alias(request * req, alias * current1, int uri_len)
{
   ...
   /* we have path_info if c == '/'... still have to check for query */
	 ...
	 if (!strncmp(req->path_info, current->fakename,
	    ...
	    memcpy(buffer, current->realname, current->real_len);
	    strlcpy(buffer + current->real_len,
		   &req->path_info[current->fake_len], (sizeof(buffer) - current->real_len));
