...
int process_logline(request * req)
{
   ...
   /* RTK: Prevent header too long make system crash */
   cnt = 0;
   while (*(++stop) == ' ')
   {
       cnt++;
       if (cnt >= MAX_HEADER_LENGTH)
       {
           fprintf(stderr, "1 URI too long %d: \"%s\"\n", MAX_HEADER_LENGTH, req->logline);
   ...
   while (*stop2 != '\0' && *stop2 != ' ')
   {
      ...
      if (cnt >= MAX_HEADER_LENGTH)
      {
          fprintf(stderr, "2 URI too long %d: \"%s\"\n", MAX_HEADER_LENGTH, req->logline);
          send_r_bad_request(req);
          return 0;
      }
   }
   ...
   if (*stop2 == ' ') {
	 ...
	 ++stop2;
      ...
      if (!memcmp(stop2, "HTTP/", 5) && sscanf(stop2, "HTTP/%u.%u", &p1, &p2) == 2) {
	 /* HTTP/{0.9,1.0,1.1} */
...
int process_header_end(server_params * params, request * req)
{
   ...
   if (NULL != req->ssl)
   {
       add_cgi_env(req, "ACCESS_TYPE", "https", 0);
   }
...
int process_option_line(request * req)
{
   ...
   log_error_time();
   fprintf(stderr, "%s:%d - len=%d Parsing \"%s\"\n", __FILE__, __LINE__, len, line);
#endif
   ...
   *value++ = '\0';		/* overwrite the : */
   ...
   while ((c = *value) && (c == ' ' || c == '\t'))
   {
      ...
      cnt++;
      if (cnt >= MAX_HEADER_LENGTH - strlen(line))
      {
          fprintf(stderr, "%s:%d - value too long \"%s\"\n", __FILE__, __LINE__, value);
          send_r_bad_request(req);
          return 0;
      }
   }
   ...
   /* RTK: Do not process warning option */
   case 'W':
      if (!memcmp(line, "WARNING", 7)) {
          break;
      }
      break;
...
void add_accept_header(request * req, char *mime_type)
{
   ...
   if (req->accept[0] == '\0')
      strlcpy(req->accept, mime_type, sizeof(req->accept));
   else {
      req->accept[l] = ',';
      req->accept[l + 1] = ' ';
