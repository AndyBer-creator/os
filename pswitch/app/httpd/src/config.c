...
long int max_ssl_connections = 10;
...
#define MAX_ARG     4
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        ...
        *--ep = 0; /* remove newline */
        ...
        fprintf(stderr, "%s args=%d '%s' '%s' '%s' '%s' '%s'\n", keyw, args,
                args >= 1 ? arg[0] : "",
                args >= 2 ? arg[1] : "",
                args >= 3 ? arg[2] : "",
                args >= 4 ? arg[3] : "",
        ...
        case STMT_TWO_ARGS:
            ...
            break;
        case STMT_THREE_ARGS:
            if (args != 3) {
                fprintf(stderr, "%s has only 3 arguments, line %d\n", keyw, line);
                return(-1);
            }
            break;
        case STMT_FOUR_ARGS:
            if (args != 4) {
                fprintf(stderr, "%s has only 4 arguments, line %d\n", keyw, line);
                return(-1);
            }
            break;
        default:
        ...
        if (cmd->action)
            (*cmd->action)(arg[0], arg[1], arg[2], arg[3], cmd->object);
    }
    return(0);
}
...
void read_config_files(void)
{
    ...
    mime_types = strdup("/etc/mime.types");
        ...
        if ((yyin = fopen("/etc/boa_login.conf", "r"))) {
            if (embedparse(yyin)) {
                fputs("Error parsing config files, exiting\n", stderr);
                exit(1);
            }
        ...
        if (webauth)
        {
            if ((yyin = fopen("/etc/boa_webauth.conf", "r"))) {
                if (embedparse(yyin)) {
                    fputs("Error parsing config files, exiting\n", stderr);
                    exit(1);
                }
                    ...
                    if ((p = strchr(temps, ' ')) != NULL) {
                        *p++ = '\0';
                        if (strcmp(temps, "passwd") == 0) {
                            strlcpy(auth_old_password, p, sizeof(auth_old_password));
                            break;
                        }
                    }
