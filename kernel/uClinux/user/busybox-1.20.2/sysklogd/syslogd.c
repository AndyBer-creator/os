...
struct globals {
	...
	char recvbuf[MAX_READ * (1 + ENABLE_FEATURE_SYSLOGD_DUP) * 128];
...
extern void rtk_timestamp_and_log(int pri, char *msg, int len, time_t now);

extern int log_ratelimit_init(unsigned interval);
extern void log_ratelimit_exit(void);
extern int log_ratelimit_add(int pri, char *msg, int len, time_t tt);
#endif
...
static void parse_syslogdcfg(const char *file)
{
	...
	while (config_read(parser, tok, 4, 3, "# \t", PARSE_NORMAL | PARSE_MIN_DIE)) {
		...
		if (!strcmp(tok[0], "global"))
			type = SYSLOG_TYPE_GLOBAL;
		else if (!strcmp(tok[0], "local-ram"))
			type = SYSLOG_TYPE_BUFFERED;
		...
		if (type == SYSLOG_TYPE_GLOBAL) {
			if (!strcmp(tok[1], "ratelimit"))
				G.ratelimit = bb_strtou(tok[2], NULL, 10);
			continue;
		}
 ...
 next:
		...
		if (type == SYSLOG_TYPE_BUFFERED || type == SYSLOG_TYPE_CONSOLE || type == SYSLOG_TYPE_FLASH) {
			/* local */
			...
			cur_rule->target.file->fp = NULL;
			cur_rule->target.file->path = xstrdup(tok[2]);

			if (type == SYSLOG_TYPE_BUFFERED) {
				cur_rule->target.file->logFileSize = 1 << CONFIG_USER_SYSLOG_RAM_SIZE;
				cur_rule->target.file->logFileRotate = 0;

				if (!access(cur_rule->target.file->path, F_OK) && !stat(cur_rule->target.file->path, &statf))
					cur_rule->target.file->size = statf.st_size;
			}
			else if (type == SYSLOG_TYPE_FLASH) {
...
#if CONFIG_RTK_PATCH
static int rtklog_systime_get(time_t *pTime)
{
	static FILE *fp = NULL;
	time_t t;

	if (!fp)
		fp = fopen(SYS_LOG_TIME_FILE, "r+");

	if (fp)
	{
		fseek(fp, 0, SEEK_SET);
		if (1 == fread(&t, sizeof(time_t), 1, fp))
		{
			*pTime = t;
		}
		else
...
static void log_locally(time_t now, char *msg, logFile_t *log_file)
{
 ...
 reopen:
		...
		if (log_file->fd >= 0)
		{
			log_file->fp = fdopen(log_file->fd, "a+");
		...
		log_file->size = log_file->isRegular ? statf.st_size : 0;
	}
...
#else
	if (log_file->logFileSize && log_file->isRegular && log_file->size > log_file->logFileSize &&
		!rotate && !fstat(log_file->fd, &statf) && S_ISREG(statf.st_mode)) {

		char *addr;
		off_t off = log_file->logFileSize >> 2;

		if (log_file->size > log_file->logFileSize)
		{
			addr = mmap(0, statf.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, log_file->fd, 0);
			...
			off += statf.st_size - log_file->logFileSize;

			/* find next '\n' */
			while (off <= statf.st_size && *(addr + off++) != '\n')
				off++;

			log_file->size = statf.st_size - off; 

			memmove(addr, addr + off, log_file->size);
			munmap(addr, statf.st_size);
			ftruncate(log_file->fd, log_file->size);
	...
	log_file->size += fprintf(log_file->fp, "%s\n", msg);
	fflush(log_file->fp);
#ifdef SYSLOGD_WRLOCK
	fl.l_type = F_UNLCK;
	fcntl(log_file->fd, F_SETLKW, &fl);
...
#if CONFIG_RTK_PATCH
void rtk_timestamp_and_log(int pri, char *msg, int len, time_t now)
{
	logRule_t *rule;
	...
	tm = localtime(&now);
	strftime(timestamp, 64, "%b %d %Y %T" , tm);

	/* preserve leading facility code */
	sprintf(G.printbuf, "<%d>*%s: %s",
			LOG_MAKEPRI(SYS_LOG_MSG_FACILITY, LOG_PRI(pri)), timestamp, msg);

	for (rule = G.log_rules; rule; rule = rule->next) {
		if (rule->enabled_facility_priomap[facility] & prio_bit) {
			switch (rule->type) {
			case SYSLOG_TYPE_BUFFERED:
			case SYSLOG_TYPE_FLASH:
				log_locally(now, G.printbuf, rule->target.file);
				break;

			case SYSLOG_TYPE_CONSOLE:
				log_locally(now, G.printbuf + 5, rule->target.file);
				break;

#if ENABLE_FEATURE_REMOTE_LOG
			case SYSLOG_TYPE_REMOTE:
				snprintf(G.printbuf, 5, "<%d>",
						LOG_MAKEPRI(rule->target.remote->facility_alias, LOG_PRI(pri)));
				G.printbuf[4] = '>';
...
void timestamp_and_log(int pri, char *msg, int len)
{
	int l = 0;
	time_t now;
	...
	/* Drop leading "syslog: " or "kernel: ", or private key */
	...
	if (!strncmp(msg, SYS_LOG_RTK_KEY, strlen(SYS_LOG_RTK_KEY)))
		msg += strlen(SYS_LOG_RTK_KEY);
	else
		return;
...
#if ENABLE_FEATURE_SYSLOGD_CFG
	/* timestamp */
	rtklog_systime_get(&now);
	/* Log message locally (to file or shared mem) */
	if (G.ratelimit > 0)
		log_ratelimit_add(pri, msg, l, now);
    else
		rtk_timestamp_and_log(pri, msg, l, now);
#endif
}
#else
...
#if CONFIG_RTK_PATCH
	if (G.ratelimit > 0)
		log_ratelimit_init(G.ratelimit);
#endif
...
#endif
 read_again:
		sz = read(sock_fd, recvbuf, MAX_READ * 128 - 1);
...
int syslogd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	*strchrnul(G.hostname, '.') = '\0';
	...
	if (!(opts & OPT_nofork)) {
		bb_daemonize_or_rexec(DAEMON_CHDIR_ROOT | DAEMON_CLOSE_EXTRA_FDS, argv);
	}
